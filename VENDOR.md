...
VH doesn't reimplement everything. A few pieces are vendored so the app works fresh off a clone — they're shipped inside the installer (`src-tauri/tauri.conf.json` → `bundle.resources: ["../vendor"]`).

| Path | Where it comes from | License | What VH uses it for |
|---|---|---|---|
