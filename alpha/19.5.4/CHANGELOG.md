...
## 19.5.4-alpha "Bridge" — 2026-09-18  [ALPHA]

Pages on top of 19.5.4 "Reach". **The released engine is untouched**: no file under
...
empty-policy behaviour were added, all 59 originals kept) · `probe/bridge.test.ts` (84) ·
`probe/memoryChannel.test.ts` (48) · `probe/beaconEvidence.test.ts` (84) ·
`probe/federationBatch.test.ts` (50).

**332 checks, 0 failed**, via `node tools/run-all-probes.mjs`.

TypeScript compiles the whole increment under `strict` + `noUncheckedIndexedAccess`:
**0 errors**.

**Mutation-tested: 10 deliberate breakages, 10 caught** — trust baseline, negotiation
intersection, wheel-handover refusal, redaction no-op, assumed refusal coverage, secret
guard, supersession, `asOf`, station handoff, and all seven policy-syntax refusal sites.
