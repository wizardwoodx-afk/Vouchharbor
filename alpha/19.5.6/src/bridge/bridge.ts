...
import { type SignedApproval, verifyApproval } from './approval';
import { TrustLedger } from './ledger';
import { type TrustPosture, pairId as makePairId } from './trust';

/* ------------------------------- negotiation ------------------------------ */

export interface CapabilityGrant {
  /** What this side is willing to make available. A declaration, not a right. */
  readonly offers: readonly BridgeCapability[];
  /** What this side can actually perform. Offering is not performing. */
...
export interface SignedEnvelope {
  ...
  readonly signer: EnvelopeSignature;
}
 ...
 */
export async function signEnvelope(envelope: BridgeEnvelope, keys: HarborKeys): Promise<SignedEnvelope> {
  if (keys.harbor !== envelope.from.harbor) {
    throw new Error(
      `refused: a harbor may only sign envelopes it sent (signing as ${keys.harbor}, envelope from ${envelope.from.harbor})`,
    );
  }
  return {
    envelope,
    signer: {
      ...
      keyId: keys.keyId,
      alg: keys.alg,
      signature: await keys.sign(utf8Bytes(canonicalEnvelope(envelope))),
    },
  };
}
...
export async function verifySignedEnvelope(signed: SignedEnvelope, registry: KeyRegistry): Promise<EnvelopeVerdict> {
  const problems = validateEnvelope(signed.envelope);
  ...
  const key = lookupKey(registry, signer.harbor, signer.keyId);
  if (key === undefined) {
    return { ok: false, reason: `no public key held for ${signer.harbor}#${signer.keyId}; an unverifiable request is refused` };
  }
  const valid = await verifySignature(key, utf8Bytes(canonicalEnvelope(envelope)), signer.signature);
  return valid
...
export const JOINT_RECEIPT_VERSION = 'vh-bridge-joint-receipt/2';

export interface PartySignature {
  readonly harbor: string;
  readonly handle: string;
  /** Which key signed. A receipt names its key so rotation is possible and provable. */
  readonly keyId: string;
  readonly alg: string;
  readonly signature: string;
}
...
export type UnsignedReceipt = Omit<JointReceipt, 'signatures'>;

/** Stable canonical bytes both sides sign. Any drift breaks verification loudly. */
export function canonicalReceipt(receipt: UnsignedReceipt): string {
  const r = receipt;
 ...
 * other's half.
 */
export async function signReceiptHalf(
  unsigned: UnsignedReceipt,
  keys: HarborKeys,
  as: PeerRef,
): Promise<PartySignature> {
  ...
  return {
    ...
    keyId: keys.keyId,
    alg: keys.alg,
    signature: await keys.sign(utf8Bytes(canonicalReceipt(unsigned))),
  };
}

export function assembleJointReceipt(unsigned: UnsignedReceipt, signatures: readonly PartySignature[]): JointReceipt {
  if (signatures.length !== 2) {
    throw new Error(`refused: a joint receipt carries two signatures, got ${signatures.length}`);
  }
  const [a, b] = signatures as readonly [PartySignature, PartySignature];
...
export interface ReceiptVerification {
  ...
  /** Parties whose signature verified against a held public key. */
  readonly signedBy: readonly string[];
  /** Parties whose signature failed, named. Never repaired. */
 ...
 */
export async function verifyJointReceipt(
  receipt: JointReceipt,
  registry: KeyRegistry,
): Promise<ReceiptVerification> {
  if (receipt.v !== JOINT_RECEIPT_VERSION) {
    return { valid: false, detail: `receipt is version ${receipt.v}; this bridge reads ${JOINT_RECEIPT_VERSION}`, signedBy: [], failures: [] };
  }
  ...
  const bytes = utf8Bytes(canonicalReceipt(unsigned));
  const signedBy: string[] = [];
  const failures: string[] = [];

  for (const sig of receipt.signatures) {
    const who = `${sig.harbor}#${sig.handle}`;
    const key = lookupKey(registry, sig.harbor, sig.keyId);
    if (key === undefined) {
    ...
    if (await verifySignature(key, bytes, sig.signature)) signedBy.push(who);
    else failures.push(`${who}: signature does not verify against the published key`);
  }
  ...
  const expectedPair = makePairId(receipt.from.harbor, receipt.from.handle, receipt.to.harbor, receipt.to.handle);
...
export type RefusalKind = 'refused_by_policy' | 'refused_by_peer' | 'approval_required' | 'disputed';

export interface PeerReplyRefusal {
  readonly status: 'refused';
  readonly cause: string;
}
export interface PeerReplyFailure {
  readonly status: 'failed';
  readonly cause: string;
}
export interface PeerReplyAccepted {
  readonly status: 'accepted';
  readonly summary: string;
  readonly answerDigest: string;
  /** The far side's half of the joint receipt, produced on the far machine. */
  readonly signature: PartySignature;
}
export type PeerReply = PeerReplyRefusal | PeerReplyFailure | PeerReplyAccepted;

export interface CrossingInput {
  readonly envelope: BridgeEnvelope;
  /** The requester's own signed envelope, when the hop arrived over a wire. */
  readonly signedEnvelope?: SignedEnvelope;
  readonly ours: CapabilityGrant;
  /** The far side's declared capability. Their posture, not our assumption. */
  readonly peerGrant: CapabilityGrant;
  readonly peerReply: PeerReply;
  readonly keys: HarborKeys;
  readonly registry: KeyRegistry;
  readonly ledger: TrustLedger;
  readonly nowIso: string;
  /** Required for a pair's first crossing when `requireHumanFirstCrossing`. */
  readonly approval?: SignedApproval;
  /** Default TRUE. The bridge refuses rather than remembering an intention. */
  readonly requireHumanFirstCrossing?: boolean;
  readonly crossings?: number;
}

export interface CrossingAnswered {
...
export interface CrossingRefused {
  readonly status: 'refused';
  readonly cause: string;
...
export interface CrossingFailed {
  readonly status: 'failed';
  readonly cause: string;
  readonly posture: TrustPosture;
  readonly statement: string;
}
export type CrossingResult = CrossingAnswered | CrossingRefused | CrossingFailed;

export function envelopePreamble(envelope: BridgeEnvelope): UnsignedReceipt {
  const pair = makePairId(envelope.from.harbor, envelope.from.handle, envelope.to.harbor, envelope.to.handle);
  return {
 ...
 */
export async function crossBridge(input: CrossingInput): Promise<CrossingResult> {
  const { envelope, keys, registry, ledger, nowIso } = input;
  const pair = makePairId(envelope.from.harbor, envelope.from.handle, envelope.to.harbor, envelope.to.handle);
  const refuse = async (
    cause: string,
    kind: RefusalKind,
    record?: { kind: 'policy-refusal' | 'peer-refusal' | 'dispute'; note: string },
  ): Promise<CrossingRefused> => {
    if (record !== undefined) {
      await ledger.attest(
        {
          pairId: pair,
  ...
  if (input.signedEnvelope !== undefined) {
    ...
    if (!verdict.ok) return refuse(verdict.reason, 'refused_by_policy');
  }
  ...
  const hop = canHop({ ...envelope, depth: envelope.depth - 1 < 0 ? 0 : envelope.depth - 1 }, nowIso);
  if (envelope.depth > 0 && !hop.ok) return refuse(hop.reason, 'refused_by_policy');
  ...
  if (envelope.depth > postureNow.maxDepth) {
    return refuse(
      `refused: this pair permits depth up to ${postureNow.maxDepth}, and this envelope is at depth ${envelope.depth}`,
      'refused_by_policy',
    );
  }
  ...
  if (humanFirst) {
    ...
    if (!crossedBefore) {
      if (input.approval === undefined) {
        ...
        return refuse(
          `refused: this would be the first crossing with ${envelope.to.handle}@${envelope.to.harbor}, and a person has to approve it. Gather an approval for pair ${pair} and envelope ${envelope.id} (nonce ${envelope.nonce}) and present it with the request.`,
          'approval_required',
        );
      }
      const approved = await verifyApproval(input.approval, registry, {
        pairId: pair,
        envelopeId: envelope.id,
        envelopeNonce: envelope.nonce,
        capability: envelope.capability,
      ...
      if (!approved.ok) return refuse(`refused: ${approved.reason}`, 'approval_required');
    }
  }
  ...
  if (!verdict.accepted) return refuse(verdict.cause, 'refused_by_policy');
  ...
  if (!input.peerGrant.canDo.includes(envelope.capability)) {
    ...
    return refuse(
      `refused: ${envelope.to.handle}@${envelope.to.harbor} cannot perform "${envelope.capability}" on a bridge; they can do ${theyCan}`,
      'refused_by_policy',
    );
  }
  ...
  if (input.peerReply.status === 'failed') {
    await ledger.attest(
      { pairId: pair, kind: 'failure', atIso: nowIso, envelopeId: envelope.id, receiptDigest: '', note: input.peerReply.cause },
      keys,
    );
  ...
  if (!theirLine.ok) {
    return refuse(
      `refused: the far side's receipt summary is not publishable — ${theirLine.reason.replace('refused: ', '')}`,
      'refused_by_policy',
    );
  }

  const theirHalf = input.peerReply.signature;
  const theirKey = lookupKey(registry, theirHalf.harbor, theirHalf.keyId);
  const theirHalfOk =
    theirKey !== undefined &&
    theirHalf.harbor === envelope.to.harbor &&
    (await verifySignature(theirKey, utf8Bytes(canonicalReceipt(unsigned)), theirHalf.signature));

  if (!theirHalfOk) {
    /* A half that does not verify is a dispute, and disputes cost the most. */
    return refuse(
      `refused: the far side's half-signature over envelope ${envelope.id} does not verify against their published key; the crossing is recorded as a dispute`,
      'disputed',
      {
        kind: 'dispute',
...
export function summarise(summary: string): { ok: boolean; reason: string } {
  if (summary.includes('\n') || summary.includes('\r')) {
  ...
  if (summary.length > SUMMARY_MAX) {
    return { ok: false, reason: `refused: a receipt summary must not exceed ${SUMMARY_MAX} characters (got ${summary.length}); a digest is not a delivery mechanism` };
  }
...
export interface ResponderInput {
  ...
  readonly ledger: TrustLedger;
  readonly nowIso: string;
  readonly answer?: { readonly summary: string; readonly digest: string };
  readonly refuseCause?: string;
  readonly requireHumanFirstCrossing?: boolean;
}

export interface ResponderAnswered {
  readonly status: 'accepted';
  readonly unsigned: UnsignedReceipt;
  readonly signature: PartySignature;
  readonly statement: string;
}
export interface ResponderRefused {
  readonly status: 'refused';
  readonly cause: string;
  readonly statement: string;
}
 ...
 */
export async function respondToCrossing(input: ResponderInput): Promise<ResponderAnswered | ResponderRefused> {
  ...
  if (!verdict.ok) return { status: 'refused', cause: `refused: ${verdict.reason}`, statement: verdict.reason };
  ...
  if (input.refuseCause !== undefined) {
    await ledger.attest(
      { pairId: pair, kind: 'policy-refusal', atIso: nowIso, envelopeId: envelope.id, receiptDigest: '', note: input.refuseCause },
      keys,
    );
  ...
  if (!own.accepted) {
    await ledger.attest(
      { pairId: pair, kind: 'policy-refusal', atIso: nowIso, envelopeId: envelope.id, receiptDigest: '', note: own.cause },
      keys,
    );
