...
import { sha256Hex } from '../src/evidence/sha256';
...
export async function runBeaconEvidenceProbe(): Promise<ProbeResult> {
  ...
  check(
    'the digest is SHA-256 over the canonical pack, recomputed independently',
    declaredDigest === `vh1:sha256:${sha256Hex(canonicalPack(packBody))}`,
  );
  check('the digest helper is SHA-256 too', digestText('abc') === `vh1:sha256:${sha256Hex('abc')}`);
  check('an answer digest can name its key', sha256Hex('abc').length === 64);
