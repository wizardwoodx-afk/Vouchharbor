...
exports.runBeaconEvidenceProbe = runBeaconEvidenceProbe;
const beacon_1 = require("../src/beacon/beacon");
const pack_1 = require("../src/evidence/pack");
...
async function runBeaconEvidenceProbe() {
    const checks = [];
    ...
    /* ------------------------------- BEACON ------------------------------- */
    const b = new beacon_1.Beacon({ stallAfterMs: 60_000, windowSize: 5 });
    ...
    b.record('tool.command', (0, beacon_1.describeCommandEvent)('sfc /verify --scope invoices', 0), T1, { detail: 'exit 0' });
    b.record('file.write', (0, beacon_1.describeFileEvent)('/work/recon.md', 4096, 'write'), T1, { detail: '4096 bytes' });
    check('events land in the window', b.snapshot(T1).window.length === 3);
    check('file events never carry contents', (0, beacon_1.describeFileEvent)('/work/recon.md', 4096, 'write').includes('contents stay on your machine'));
    check('a file event states the size', (0, beacon_1.describeFileEvent)('/work/recon.md', 4096, 'write').includes('4096 bytes'));
    ...
    check('the stall event is labelled in the feed', (0, beacon_1.unattendedLabel)(b.snapshot(T2).window.find((e) => e.kind === 'run.stalled')) === 'nothing happened for a while');
    ...
    const b2 = new beacon_1.Beacon({ stallAfterMs: 60_000, windowSize: 5 });
    b2.record('run.started', 'run', T0);
    for (let i = 0; i < 12; i += 1)
        b2.record('file.read', `read file ${i}`, T1);
    ...
    check('the default window is a window, not a log', beacon_1.DEFAULT_BEACON_OPTIONS.windowSize <= 200);
    b2.record('run.finished', 'mission complete', T2);
    check('finishing turns the beacon dark', b2.snapshot(T2).state === 'dark');
    /* redaction */
    check('an api key in a command is redacted', !(0, beacon_1.redactCommand)('curl -H "key: sk-abcdefghijklmnop"').includes('sk-abcdefghijklmnop'));
    check('a flagged argument is redacted', (0, beacon_1.redactCommand)('deploy --token abcdef123456').includes('[redacted]'));
    check('an ordinary command survives redaction', (0, beacon_1.redactCommand)('npm run verify').includes('npm run verify'));
    check('a command event reports the exit code', (0, beacon_1.describeCommandEvent)('ls -la', 0).includes('exit 0'));
    check('a running command says so', (0, beacon_1.describeCommandEvent)('ls -la', undefined).includes('still running'));
    /* a credential cannot reach a window by any of the shapes it arrives in */
    check('a github token is redacted', !(0, beacon_1.redactCommand)('git push https://ghp_ABCDEFGHIJ0123456789@github.com/x/y').includes('ghp_ABCDEFGHIJ0123456789'));
    check('a bearer token is redacted', !(0, beacon_1.redactCommand)('curl -H "Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.payload.sig"').includes('eyJhbGciOiJIUzI1NiJ9.payload.sig'));
    check('a private key header is redacted', !(0, beacon_1.redactCommand)('cat /tmp/-----BEGIN RSA PRIVATE KEY-----').includes('BEGIN RSA PRIVATE KEY'));
    check('the flag in front of a redacted value survives', (0, beacon_1.redactCommand)('deploy --token abcdef123456').includes('--token'));
    check('redaction leaves an ordinary line alone', (0, beacon_1.redactCommand)('ls -la /work && npm run verify') === 'ls -la /work && npm run verify');
    /* the icon is ours */
    check('the beacon icon is named beacon', beacon_1.BEACON_ICON.name === 'beacon');
    check('the icon covers every state', Object.keys(beacon_1.BEACON_ICON.states).length === 6);
    check('the dark state has no beam', beacon_1.BEACON_ICON.states.dark.beamOpacity === 0);
    check('the icon explains itself', beacon_1.BEACON_ICON.meaning.includes('whether anything is still moving'));
    check('BeaconError is identifiable', (() => {
        try {
            b.record('run.started', '', T0);
            return false;
        }
        catch (e) {
            return e instanceof beacon_1.BeaconError;
        }
    })());
    /* ---------------------------- EVIDENCE PACK --------------------------- */
    const input = {
        ...
        versions: [{ surface: 'app', value: '19.5.6-alpha' }, { surface: 'engine', value: '19.5.6-alpha' }],
        retentionDays: 365,
        notes: ['Bridge opened with harbor-b for settlement verification.'],
    };
    const pack = (0, pack_1.buildEvidencePack)(input);
    check('the pack carries its version', pack.v === pack_1.EVIDENCE_PACK_VERSION);
    ...
    check('the pack maps EU AI Act Art. 12', pack_1.COMPLIANCE_MAP.some((c) => c.clause.includes('Art. 12')));
    ...
    /* Pin the algorithm, not just the behaviour. A checksum changes when a pack is
       tampered with too, so "tamper detected" alone does not prove SHA-256 — this
       recomputes the digest independently over the pack's canonical bytes. */
    const { digest: declaredDigest, ...packBody } = pack;
    check('the digest names its algorithm', /^vh1:sha256:[0-9a-f]{64}$/.test(declaredDigest), declaredDigest);
    check('the digest is SHA-256 over the canonical pack, recomputed independently', declaredDigest === `vh1:sha256:${(0, sha256_1.sha256Hex)((0, pack_1.canonicalPack)(packBody))}`);
    check('the digest helper is SHA-256 too', (0, pack_1.digestText)('abc') === `vh1:sha256:${(0, sha256_1.sha256Hex)('abc')}`);
    check('an answer digest can name its key', (0, sha256_1.sha256Hex)('abc').length === 64);
    const verify = (0, pack_1.verifyEvidencePack)(pack, pack.digest);
    ...
    const wrongDigest = (0, pack_1.verifyEvidencePack)(pack, 'vh1:00000000');
    check('a wrong expected digest fails', wrongDigest.valid === false);
    const tampered = { ...pack, retentionDays: 30 };
    check('a tampered pack fails verification', (0, pack_1.verifyEvidencePack)(tampered, tampered.digest).valid === false);
    /* a refusal with no named rule lowers the measured coverage — the number is
       computed, not asserted */
    const undocumented = (0, pack_1.buildEvidencePack)({
        ...input,
        decisions: [
            ...input.decisions,
    ...
    const gappy = (0, pack_1.buildEvidencePack)({
        ...input,
        receipts: [
    ...
    check('a gappy pack still verifies as intact', (0, pack_1.verifyEvidencePack)(gappy, gappy.digest).valid === true);
    const cover = (0, pack_1.renderCoverNote)(pack);
    ...
    const gappyCover = (0, pack_1.renderCoverNote)(gappy);
    ...
    check('the digest helper is stable', (0, pack_1.digestText)('abc') === (0, pack_1.digestText)('abc'));
    check('the digest helper separates inputs', (0, pack_1.digestText)('abc') !== (0, pack_1.digestText)('abd'));
