...
**Doc version: 19.5.6-alpha "Bridge".** This file exists so that "properly aligned, wired and connected" is something you can *audit* rather than something you have to take on faith. Every row states where an addition sits, what it reads, what reads it, and what it deliberately does not touch.
...
| Agent Bridge | `src/bridge/{envelope,keys,approval,ledger,trust,bridge}.ts` | envelopes, pair ledger, peer grants, public keys | the two harbors | `VH_BRIDGE` | `probe/bridge.test.ts` |
| Key material | `src/bridge/keys.ts` | the platform's Web Crypto | each harbor holds its own private key | (with the bridge) | `probe/bridge.test.ts` |
| Human approval | `src/bridge/approval.ts` | the owner's key, the envelope nonce | a signed record from a person | (with the bridge) | `probe/bridge.test.ts` |
| Trust ledger | `src/bridge/ledger.ts` | signature-verified crossing records | both harbors, one copy each | (with the bridge) | `probe/bridge.test.ts` |
...
| Release packer | `tools/pack-release.mjs` | the repo tree, the probe runner | — | — | read-back check + `--list` |
| Probe twins | `tools/compile-probe-twins.mjs` → `dist-verify/` | `src/`, `catalog/`, `probe/` | — | — | the twins **are** the check |
| SHA-256 | `src/evidence/sha256.ts` | nothing | — | (with the pack) | `probe/sha256.test.ts` |
...
**Precedence 1b — verify the pack before you enable anything.** `node dist-verify/tools/run-all-probes.mjs`
runs on plain Node with no loader and no dependencies, so the numbers in the release
notes are numbers you produce, not numbers you are told. If that command does not run,
the pack is not installable, whatever else it says.
...
| Surface | Where | Value |
|---|---|---|
| Version constant | `docs/VERSION.ts` | `19.5.6-alpha` |
| Display form | `docs/VERSION.ts` → `VERSION_DISPLAY` | `19.5.4 [Alpha]` |
| Codename | `docs/VERSION.ts` → `CODENAME` | `Bridge` |
| Machine manifest | `docs/MANIFEST.json` | `19.5.6-alpha` |
| Wiring (this file) | `docs/ALIGNMENT.md` headings | `19.5.6-alpha "Bridge"` |
| Feature sheet | `FEATURES.md` | header + §12 verification results |
| Changelog | `CHANGELOG.md` | `19.5.6-alpha "Bridge"` |
