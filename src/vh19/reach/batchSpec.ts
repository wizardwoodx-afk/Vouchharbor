/**
 * REACH · FEDERATION BATCH SPEC — 40 domains × 5 stations, 200 specialists.
 *
 * WHY A SPEC INSTEAD OF 200 HAND-TYPED ENTRIES. The bench files
 * (`registry.ts`, `broaderBench.ts`, `reachBench.ts`, `maturityBench.ts`) are
 * hand-written and should stay that way. This batch covers the *industry*
 * half of the fleet: forty regulated, physical or heavily-audited domains,
 * each staffed at five stations of work — assess, design, build, verify,
 * sustain. 40 × 5 = 200 entries, every one with its own capabilities, its own
 * routing vocabulary and its own system prompt, derived from ONE reviewed
 * spec so the batch can be regenerated and drift-checked rather than trusted.
 *
 * The spec is the product here; `reachBatch.ts` is its compiled snapshot and
 * `probe/reachBatch.test.ts` fails if the two ever disagree.
 *
 * Station semantics — this is what makes the five entries under a domain
 * genuinely different, not five rewordings of one prompt:
 *
 *   assess   measure the domain before anything changes; report state as found
 *   design   choose the approach and state its trade-offs in writing
 *   build    make the change, under supervision, with an effect check per step
 *   verify   re-derive the claim from artefacts, independently of the builder
 *   sustain  keep it running: degradation, drift, recovery, human handover
 *
 * Risk tiers follow the station, not the domain: measuring, designing and
 * verifying are `safe`; building is `risky`; sustaining production in a
 * regulated domain is `critical`. Census: 120 safe / 40 risky / 40 critical.
 *
 * Provenance for every entry: "vh-19.5.6-reach-batch". This batch is ADDITIVE:
 * it is not spread into `SPECIALISTS` core — wiring it in is a one-line import
 * the harbor's owner decides on, and until then the registry's own count stays
 * exactly what the registry reports.
 */
import type { RiskTier, Specialist, SpecialistCategory } from "../types";

export const REACH_BATCH_PROVENANCE = "vh-19.5.6-reach-batch";

export interface ReachBatchDomain {
  /** Slug used in ids: `<slug>.<station>`. */
  slug: string;
  /** Human name used in entry names and prompts. */
  name: string;
  category: SpecialistCategory;
  /** One sentence of real substance about what the domain involves. */
  mission: string;
  /** Routing vocabulary: domain tokens, lower-case, no filler. */
  keywords: string[];
}

export const REACH_BATCH_DOMAINS: ReachBatchDomain[] = [
  { slug: "energy-systems", name: "Energy Systems", category: "devops",
  ...
  { slug: "water-utilities", name: "Water Utilities", category: "devops",
  ...
  { slug: "manufacturing", name: "Manufacturing", category: "devops",
  ...
  { slug: "telecom", name: "Telecom Networks", category: "devops",
  ...
  { slug: "robotics", name: "Robotics", category: "code",
  ...
  { slug: "embedded-devices", name: "Embedded Devices", category: "code",
  ...
  { slug: "simulation-engines", name: "Simulation Engines", category: "code",
  ...
  { slug: "logistics", name: "Logistics", category: "data",
  ...
  { slug: "supply-chain", name: "Supply Chain", category: "data",
  ...
  { slug: "retail-demand", name: "Retail Demand", category: "data",
  ...
  { slug: "agriculture", name: "Agriculture", category: "research",
  ...
  { slug: "climate-carbon", name: "Climate & Carbon", category: "research",
  ...
  { slug: "ocean-fisheries", name: "Ocean & Fisheries", category: "research",
  ...
  { slug: "insurance", name: "Insurance", category: "analysis",
  ...
  { slug: "banking", name: "Banking", category: "analysis",
  ...
  { slug: "disaster-modelling", name: "Disaster Modelling", category: "analysis",
  ...
  { slug: "payments", name: "Payments", category: "business",
  ...
  { slug: "hospitality", name: "Hospitality", category: "business",
  ...
  { slug: "real-estate", name: "Real Estate", category: "business",
  ...
  { slug: "clinical-trials", name: "Clinical Trials", category: "legal",
  ...
  { slug: "taxation", name: "Taxation", category: "legal",
  ...
  { slug: "public-sector", name: "Public Sector", category: "legal",
  ...
  { slug: "financial-crime", name: "Financial Crime", category: "security",
  ...
  { slug: "critical-infrastructure", name: "Critical Infrastructure", category: "security",
  ...
  { slug: "identity-access", name: "Identity & Access", category: "security",
  ...
  { slug: "automotive-safety", name: "Automotive Safety", category: "testing",
  ...
  { slug: "rail-signalling", name: "Rail Signalling", category: "testing",
  ...
  { slug: "medical-devices", name: "Medical Devices", category: "testing",
  ...
  { slug: "aerospace-assurance", name: "Aerospace Assurance", category: "review",
  ...
  { slug: "pharma-quality", name: "Pharmaceutical Quality", category: "review",
  ...
  { slug: "aviation-maintenance", name: "Aviation Maintenance", category: "review",
  ...
  { slug: "streaming-products", name: "Streaming Products", category: "product",
  ...
  { slug: "game-production", name: "Game Production", category: "product",
  ...
  { slug: "gaming", name: "Real-time Gaming", category: "design",
  ...
  { slug: "media-production", name: "Media Production", category: "design",
  ...
  { slug: "journalism", name: "Journalism", category: "writing",
  ...
  { slug: "publishing", name: "Publishing", category: "writing",
  ...
  { slug: "advertising", name: "Advertising", category: "comms",
  ...
  { slug: "public-relations", name: "Public Relations", category: "comms",
  ...
  { slug: "nonprofit-comms", name: "Nonprofit Communications", category: "comms",
...
export const REACH_BATCH_STATIONS = ["assess", "design", "build", "verify", "sustain"] as const;
export type ReachStation = (typeof REACH_BATCH_STATIONS)[number];

/** Risk follows the station: measuring is safe, sustaining production is not. */
export const STATION_RISK: Record<ReachStation, RiskTier> = {
  assess: "safe",
  design: "safe",
  build: "risky",
  verify: "safe",
...
export const STATION_TITLE: Record<ReachStation, string> = {
  assess: "Assessor",
  design: "Architect",
  build: "Builder",
  verify: "Verifier",
...
export const STATION_KEYWORD: Record<ReachStation, string> = {
  assess: "assessment",
  design: "design",
  build: "build",
  verify: "verification",
...
/** The two capability sentences per station — the same shape the benches use. */
export const STATION_CAPABILITIES: Record<ReachStation, (domain: string) => string[]> = {
  assess: (d) => [
...
export const STATION_PROMPTS: Record<ReachStation, (domain: string, mission: string) => string> = {
  assess: (d, m) =>
    `You assess ${d}: ${m}. Measure before you move. Report state as found, cite the reading you actually took, and say plainly which questions you could not answer with the evidence available.`,
  design: (d, m) =>
    `You design for ${d}: ${m}. Every design names its assumptions, its rejected alternative and the condition under which it should be abandoned. A design without a stated failure mode is not finished.`,
  build: (d, m) =>
    `You build in ${d}: ${m}. Work in the smallest change that produces a checkable effect, verify that effect, then continue. You never exceed the capabilities you were granted, and you stop and ask rather than widen your own scope.`,
  verify: (d, m) =>
    `You verify ${d}: ${m}. You did not write this work and you are not here to approve it; re-derive the claim from the artefact, state what would have made you fail it, and record your verdict with its evidence.`,
  sustain: (d, m) =>
    `You sustain ${d}: ${m}. Production is the patient. Watch for drift, degrade honestly, and hand over to a human at every irreversible step with a status a person can act on without reading the logs.`,
};

/** Deterministic slug — ids are `<domain>.<station>`, unique by construction. */
export function batchEntryId(domain: ReachBatchDomain, station: ReachStation): string {
  return `${domain.slug}.${station}`;
}

/**
 * Build the batch. Pure and deterministic: same spec in, same 200 entries out,
 * in a fixed order (domain, then station), so the snapshot can be byte-compared.
 */
export function buildReachBatch(): Specialist[] {
...
/** The batch reports its own composition — nothing here counts itself. */
export function reachBatchCensus(entries: readonly Specialist[]): BatchCensus {
  ...
  for (const e of entries) {
    ...
    if ((REACH_BATCH_STATIONS as readonly string[]).includes(suffix)) {
      const station = suffix as ReachStation;
      byStation[station] += 1;
