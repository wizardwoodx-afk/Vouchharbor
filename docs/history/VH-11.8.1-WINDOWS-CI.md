# Windows CI Test Report — VH 11.8.1

> Recorded on: 2026-09-03
> Machine: Windows (D:\VH)
> Version tested: **VH 11.8.1** ("The Turn-Limit Truth + Environment Hardening")
...
The VH 11.8.1 native Tauri desktop app builds, type-checks, compiles, runs, and
...
These are **the same three failures on every Windows machine** since VH 11.0.
They are documented in `docs/history/VH-11.8-UPGRADE.md` §6 as known platform limits, not
...
From a clean checkout of the `wizardwoodx-afk/VH-Desktop` repo at commit
`48ae70d` on `main`:
...
- The probes that fail here are documented in `VH-11.8-UPGRADE.md` §6 as
