# Vouch Harbor 19.7.10.1 — BUILD NATIVE (Windows 11)
...
| Platform | Output |
|---|---|
| Windows | `src-tauri/target/release/bundle/nsis/Vouch Harbor_19.7.10.1_x64-setup.exe` |
...
> **Identity (19.6.0):** the active native namespace is Vouch Harbor —
> `vh.sqlite`, the `vh-desktop` keychain service and `vh://event` events.
> Pre-19.5.6 builds used `mj.sqlite` / `mj-desktop`: the app migrates the
...
| `tauri build` → NSIS installer | ok — `Vouch Harbor_19.7.10.1_x64-setup.exe`, installs per-user |
