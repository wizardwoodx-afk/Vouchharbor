# Velvet Hand — BUILD NATIVE (Windows 11)

Velvet Hand is a **Tauri v2 desktop application for Windows 11**. It is not a website and it does not need a server.
...
| Platform | Output |
|---|---|
| Windows | `src-tauri/target/release/bundle/nsis/Velvet Hand_x64-setup.exe` |
...
> **Identity:** the active native namespace is Velvet Hand —
...
| `tauri build` → NSIS installer | ok — `Velvet Hand_x64-setup.exe`, installs per-user |
...
| `tauri.conf.json` | valid JSON, `frontendDist: ../dist`, `identifier: com.velvethand.app`, bundle target nsis |
