...
const TEAM_A = teamIdFor(["Harshen", "Qwen"]);
...
test("teamEvolve — the team itself learns, with every member's consent", async () => {
  ...
  const partial = await approveTeamEvolution(TEAM_A, prop.proposal.id, [{ memberId: "harshen", approved: true, at: new Date().toISOString() }]);
