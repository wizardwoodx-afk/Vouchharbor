...

{
  ...
  ok(!/[^\w./-]/.test(p.branch.replace("vh/rate-limiter/", "")), `an awkward seat id still yields a legal branch: ${p.branch}`);
...

{
  ...
  ok(/Generated by VH/.test(claude.contents), "and is marked generated, so nobody hand-edits it");
}
...

{
  ...
  ok(v.some((x) => /WARNING/.test(x.reason)), "and VH says so loudly");
...

{
  ...
  ok(free.problems.some((p) => /2 different CLIs will write/.test(p)), "and VH still names the reconciliation cost");
}
