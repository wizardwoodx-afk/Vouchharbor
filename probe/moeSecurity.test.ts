/**
 * MoE security boundary probe (17.10.11).
 *
 * The router is fed by a catalog of 158 third-party MIT prompt files and the
 * provider layer holds live API keys. That combination is the exfiltration path:
 * change a config file, get a key. So the guards here are asserted, not assumed.
 *
 * Covers: base-url allowlisting (SSRF/key-theft), secret redaction at the
 * boundary, per-run cost caps, and the A2A rule that a sender's own risk label is
 * never believed by the receiver.
 */
import {
  BudgetGuard,
  buildChatRequest,
...
ok("Gemini uses the documented v1beta/openai compatibility surface",
  gemini.openaiBaseUrl === "https://generativelanguage.googleapis.com/v1beta/openai", gemini.openaiBaseUrl);
...
ok("provider confusion is caught: an openai base cannot masquerade as anthropic",
...
const guard = new BudgetGuard(0.05);
ok("cost is computed from the provider's own table", costUsd(openai, { inputTokens: 1_000_000, outputTokens: 0 }) === openai.pricePerMTokens.input);
ok("local models cost zero", costUsd(providerById("openai-compatible")!, { inputTokens: 5000, outputTokens: 5000 }) === 0);
ok("under the cap, dispatch continues", (() => { const g = new BudgetGuard(1); g.add({ inputTokens: 1000, outputTokens: 1000 }, openai); return g.over() === false; })());
ok("over the cap, the run stops", (() => {
  const g = new BudgetGuard(0.0001);
  g.add({ inputTokens: 200_000, outputTokens: 200_000 }, anthropic);
  return g.over() === true && /EXCEEDED/.test(g.describe());
})());
ok("the cap is a refusal in words, not a silent truncation", (() => {
  const g = new BudgetGuard(0.0001);
  g.add({ inputTokens: 900_000, outputTokens: 900_000 }, gemini);
  return /of a \$0\.0001 cap/.test(g.describe());
})());

section("4. risk asserted by a sender is never accepted as truth");
// mirrors the A2A receiver rule: the grading can only go UP, never down
const verdict = (asserted: "safe" | "risky" | "critical", own: "safe" | "risky" | "critical") => {
  const rank = { safe: 0, risky: 1, critical: 2 } as const;
