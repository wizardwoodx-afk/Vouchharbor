...
function agentsMdForMission(m2) {
  ...
  lines.push("## Context");
  lines.push("");
  lines.push(`This workspace is being worked on by VH mission \`${m2.missionId}\`.`);
...
function asKV(store) {
  ...
  const m2 = store;
  return { get: (k) => m2.get(k) ?? null, set: (k, v) => void m2.set(k, v) };
}
...
var MissionRuntime = class {
  ...
  async run() {
    ...
    try {
      const res2 = durableResume(this, this.durableKV);
      if (res2.ok) this.transition("RUNNING", `Durable resume: ${res2.completedNodeIds.length} completed nodes restored (snapshot saved ${res2.savedAt}) \u2014 finished work is not repeated.`);
    } catch {
    }
    let guard = 0;
