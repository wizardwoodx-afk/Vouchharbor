...
var prov = { kind: "openai-compatible", baseUrl: "https://api.openai.com/v1", apiKey: "sk-test-abcdefgh123456789", model: "gpt-test" };
var MULTI_TEXT = "write unit tests for the typescript parser and review the code changes";
var SYNTH_MARKER = "single synthesized domain result";
test("captain synthesis \u2014 reasoned collaboration over real executions (19.3.0)", async () => {
  ...
  const captain2 = getCaptain("captain.code");
  assert.ok(captain2);
  const sys = buildSynthesisSystem(captain2);
  ...
  let memberNo = 0;
  const calls = [];
  const synthFetch = (async (_input, init) => {
    ...
    if (body.includes("Rank these specialist ids")) content = "re-rank noise";
      ...
      memberNo++;
      return new Response(JSON.stringify({ choices: [{ message: { content: `member work ${memberNo === 1 ? "A: coverage is 85% as of 2026-09-10 per https://example.org/a" : `B: coverage is 90% \u2014 tests pass`}` } }] }), { status: 200 });
    }
  ...
  check("synthesis exists and is its own receipted record", multi.synthesis !== void 0 && /^[0-9a-f]{64}$/.test(multi.synthesis?.digest ?? ""), multi.synthesis?.digest);
  ...
  memberNo = 0;
  const failSynthFetch = (async (_input, init) => {
    const body = String((init ?? {}).body ?? "");
  ...
  check("no synthesis record exists", failedSynth.synthesis === void 0);
  ...
  memberNo = 0;
  const partialFetch = (async (_input, init) => {
    const body = String((init ?? {}).body ?? "");
    if (body.includes("Rank these specialist ids")) return new Response(JSON.stringify({ choices: [{ message: { content: "re-rank noise" } }] }), { status: 200 });
  ...
  check("with one executed member, no synthesis is attempted", partial.synthesis === void 0);
  ...
  const single = await askVH19(
    ...
    { provider: prov, fetchImpl: (async () => new Response(JSON.stringify({ choices: [{ message: { content: "SYNACK: one clean answer." } }] }), { status: 200 })), gate: async () => ({ approved: true }) }
  );
  check("the route really is single-member", single.specialistIds.length === 1, single.specialistIds);
  check("single member \u21D2 executed answer with NO synthesis field", single.outcome === "answered" && single.synthesis === void 0);
  console.log(`
