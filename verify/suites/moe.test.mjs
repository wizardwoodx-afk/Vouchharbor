...
function selectCrew(decision, request) {
  ...
  const ranked = [...decision.selected].sort((a, b2) => b2.score - a.score || a.id.localeCompare(b2.id));
...
var genSrc = fs.readFileSync(path.join(ROOT, "src/vh19/generalist.ts"), "utf8");
function main() {
  ...
  ok("top-ranked expert always admitted", decision.selected[0].id === [...ranked.selected].sort((a, b2) => b2.score - a.score)[0].id);
  ...
  ok("MoE never invents candidates", decision.selected.every((c) => ranked.selected.some((r2) => r2.id === c.id)));
  ...
  ok(
    "generalist imports and applies selectCrew to the routed decision",
    genSrc.includes("selectCrew(routed, text)") && genSrc.includes("moeLine(moeReport)")
  );
  ok(
    "MoE runs on BOTH paths (provider and no-provider) \u2014 after the if/else, before team preference",
    genSrc.indexOf("const m = selectCrew(routed, text);") > genSrc.indexOf("routed = routeDeterministic(text);") && genSrc.indexOf("const m = selectCrew(routed, text);") < genSrc.indexOf("applyTeamPreference(args.team.id")
  );
  ...
  ok(
    "a security/code task assembles its crew from security/code categories (MoE spans ALL 16 categories, not just the new 700)",
    sec.decision.selected.length >= 1 && [...secCats].every((c) => ["security", "code", "review", "testing"].includes(c)) && SPECIALISTS.filter((x) => x.category === "security").length > 100
  );
  const poolIds = new Set(SPECIALISTS.map((x) => x.id));
  ok(
    "no provenance filter exists \u2014 the gate sees every established specialist equally",
