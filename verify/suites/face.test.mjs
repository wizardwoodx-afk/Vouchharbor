...
var require_react_production_min = __commonJS({
  "node_modules/react/cjs/react.production.min.js"(exports) {
    ...
    function M3(a, b2, e2) {
      ...
      if (null != b2) for (d2 in void 0 !== b2.ref && (h = b2.ref), void 0 !== b2.key && (k = "" + b2.key), b2) J2.call(b2, d2) && !L2.hasOwnProperty(d2) && (c[d2] = b2[d2]);
    ...
    function R2(a, b2, e2, d2, c) {
      ...
      if (h) return h = a, c = c(h), a = "" === d2 ? "." + Q2(h, 0) : d2, I2(c) ? (e2 = "", null != a && (e2 = a.replace(P3, "$&/") + "/"), R2(c, b2, e2, "", function(a2) {
        return a2;
      })) : null != c && (O2(c) && (c = N2(c, e2 + (!c.key || h && h.key === c.key ? "" : ("" + c.key).replace(P3, "$&/") + "/") + a)), b2.push(c)), 1;
      h = 0;
      d2 = "" === d2 ? "." : d2 + ":";
      ...
      else if (f = A2(a), "function" === typeof f) for (a = f.call(a), g = 0; !(k = a.next()).done; ) k = k.value, f = d2 + Q2(k, g++), h += R2(k, b2, e2, f, c);
...
var require_react_jsx_runtime_production_min = __commonJS({
  "node_modules/react/cjs/react-jsx-runtime.production.min.js"(exports) {
    ...
    function q(c, a, g) {
      ...
      for (b2 in a) m.call(a, b2) && !p.hasOwnProperty(b2) && (d2[b2] = a[b2]);
      if (c && c.defaultProps) for (b2 in a = c.defaultProps, a) void 0 === d2[b2] && (d2[b2] = a[b2]);
      return { $$typeof: k, type: c, key: e2, ref: h, props: d2, _owner: n.current };
    }
    exports.Fragment = l;
    exports.jsx = q;
    exports.jsxs = q;
...
// probe/face.test.ts
import { describe, it as it2 } from "node:test";
import assert from "node:assert";
import * as fs from "node:fs";
import * as path from "node:path";

// node_modules/blobatar/dist/index.js
function I({ l: t, c: e2, h: n }) {
  let a = n * Math.PI / 180, o = e2 * Math.cos(a), r = e2 * Math.sin(a), s = t + 0.3963377774 * o + 0.2158037573 * r, c = t - 0.1055613458 * o - 0.0638541728 * r, i2 = t - 0.0894841775 * o - 1.291485548 * r, l = s * s * s, m = c * c * c, u2 = i2 * i2 * i2;
  return [4.0767416621 * l - 3.3077115913 * m + 0.2309699292 * u2, -1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * u2, -0.0041960863 * l - 0.7034186147 * m + 1.707614701 * u2];
}
var N = (t) => t.every((e2) => e2 >= -1e-4 && e2 <= 1.0001);
function Q(t) {
  let e2 = I(t);
  if (!N(e2)) {
    let n = 0, a = t.c;
  ...
  return e2.map((n) => Math.min(1, Math.max(0, n)));
}
function V(t) {
  let [e2, n, a] = Q(t);
  return 0.2126 * e2 + 0.7152 * n + 0.0722 * a;
}
function O(t, e2) {
  let n = V(t), a = V(e2);
...
function E(t, e2, n) {
  if (O(t, e2) >= n) return t;
  let a = t.l >= e2.l ? 1 : -1;
  for (let s of [a, -a]) {
    let c = { ...t };
    for (let i2 = 0; i2 < 60; i2++) {
      if (c.l = Math.min(1, Math.max(0, c.l + s * 0.02)), O(c, e2) >= n) return c;
      if (c.l === 0 || c.l === 1) break;
    }
  }
  let o = { ...t, l: 0, c: 0 }, r = { ...t, l: 1, c: 0 };
  return O(o, e2) >= O(r, e2) ? o : r;
}
function B(t) {
  return "#" + Q(t).map((e2) => {
    let n = e2 <= 31308e-7 ? 12.92 * e2 : 1.055 * Math.pow(e2, 0.4166666666666667) - 0.055;
...
var It = (t) => U.find(([e2]) => t < e2)?.[1] ?? U[0][1];
var G = { l: 0.145, c: 0, h: 0 };
var J = 1.5;
var vt = (t, e2) => {
  let n = It(e2), a = E({ l: n.l, c: n.c, h: t }, G, J);
  return { bg: { l: 0.965, c: 0.01, h: t }, head: a, eye: a.l >= 0.5 ? { l: 0.17, c: 0.02, h: t } : { l: 0.97, c: 0.012, h: t } };
};
var W = [["head", "bg", 1.25], ["eye", "head", 4.5]];
function tt(t, e2 = true, n = 0) {
  let a = vt(t, n);
  if (e2) for (let [o, r, s] of W) a[o] = E(a[o], a[r], s);
  return a;
}
function v(t, e2 = true, n = 0) {
  let a = tt(t, e2, n), o = {};
  for (let r in a) o[r] = B(a[r]);
  return o;
}
var y = (t) => {
  let e2 = Math.round(t * 100) / 100;
  return Object.is(e2, -0) ? "0" : String(e2);
};
function P({ cx: t, cy: e2, rx: n, ry: a, n: o = 4, rot: r = 0 }) {
  let s = Math.min(1, (8 * Math.pow(2, -1 / o) - 4) / 3), c = n, i2 = a, l = c * s, m = i2 * s, u2 = [[c, 0], [c, m], [l, i2], [0, i2], [-l, i2], [-c, m], [-c, 0], [-c, -m], [-l, -i2], [0, -i2], [l, -i2], [c, -m], [c, 0]], b2 = r * Math.PI / 180, p = Math.cos(b2), d2 = Math.sin(b2), h = (x2) => {
    let [g, k] = u2[x2];
    return `${y(t + g * p - k * d2)} ${y(e2 + g * d2 + k * p)}`;
  }, f = `M${h(0)}`;
  for (let x2 = 1; x2 < 13; x2 += 3) f += `C${h(x2)} ${h(x2 + 1)} ${h(x2 + 2)}`;
  return f + "Z";
}
function et(t, e2, n, a, o, r = 0) {
  let s = o.length, c = r * Math.PI / 180, i2 = o.map((u2, b2) => {
    let p = c + 2 * Math.PI * b2 / s;
    return [t + n * u2 * Math.cos(p), e2 + a * u2 * Math.sin(p)];
  }), l = (u2) => i2[(u2 % s + s) % s], m = `M${y(l(0)[0])} ${y(l(0)[1])}`;
  for (let u2 = 0; u2 < s; u2++) {
    let [b2, p] = l(u2 - 1), [d2, h] = l(u2), [f, x2] = l(u2 + 1), [g, k] = l(u2 + 2);
    m += `C${y(d2 + (f - b2) / 6)} ${y(h + (x2 - p) / 6)} ${y(f - (g - d2) / 6)} ${y(x2 - (k - h) / 6)} ${y(f)} ${y(x2)}`;
  }
  return m + "Z";
}
function nt({ cx: t, cy: e2, rx: n, ry: a, sides: o, round: r = 0.3, rot: s = 0 }) {
  let c = r > 0 ? r < 1 ? r / 2 : 0.5 : 0, i2 = s * Math.PI / 180 - Math.PI / 2, l = Array.from({ length: o }, (p, d2) => {
    let h = i2 + 2 * Math.PI * d2 / o;
    return [t + n * Math.cos(h), e2 + a * Math.sin(h)];
  }), m = (p) => l[(p % o + o) % o], u2 = (p, d2) => {
    let [h, f] = m(p), [x2, g] = m(d2);
    return `${y(h + (x2 - h) * c)} ${y(f + (g - f) * c)}`;
  }, b2 = `M${u2(0, -1)}`;
  for (let p = 0; p < o; p++) {
    let [d2, h] = m(p);
    if (b2 += `Q${y(d2)} ${y(h)} ${u2(p, p + 1)}`, c < 0.5) b2 += `L${u2(p + 1, p)}`;
  }
  return b2 + "Z";
}
function rt(t, e2, n, a) {
  let o = y(t - n), r = y(t + n);
  return `M${o} ${y(e2 - a)}H${r}V${y(e2 + a)}H${o}Z`;
}
function ot(t, e2, n, a, o) {
  let r = Math.max(1.05, o), s = n * Math.sqrt(1 - 1 / (r * r)), c = e2 - a / r, i2 = e2 - r * a, l = s * 0.14, m = c + 0.86 * (i2 - c);
  return `M${y(t - s)} ${y(c)}L${y(t - l)} ${y(m)}Q${y(t)} ${y(i2)} ${y(t + l)} ${y(m)}L${y(t + s)} ${y(c)}Z`;
}
function R(t, e2) {
  for (let n = 0; n < e2.length; n++) t = Math.imul(t ^ e2[n], 3432918353), t = t << 13 | t >>> 19;
  return t;
}
function Rt(t) {
...
function ct(t, e2 = true) {
  let n = e2 ? st(t) : t;
  return R(1779033703 ^ n.length, at.encode(n));
}
function A(t, e2) {
  return Rt(R(R(t, Uint8Array.of(255)), at.encode(e2))) / 4294967296;
}
function F(t, e2 = true, n) {
  let a = ct(t, e2), o = (r) => {
...
var j = (t, e2) => e2?.tint ? e2.tint(t, e2.p) : t;
var it = (t, e2) => e2 ? `<g transform="${e2}">${t}</g>` : t;
var At = (t) => t.replace(/[&<>]/g, (e2) => e2 === "&" ? "&amp;" : e2 === "<" ? "&lt;" : "&gt;");
function T(t, e2) {
  let n = F(t, e2.normalize ?? true, e2.traits);
  return { t: n, palette: { ...v(e2.hue ?? n.num("hue", 0, 360), e2.contrast ?? true, e2.tone ?? n("tone")), ...e2.palette } };
}
var Ft = (t) => t.title ? `<title>${At(t.title)}</title>` : "";
function L(t, e2, n) {
  let a = e2.background ?? t.background;
  if (a === false) return;
...
function ut(t) {
  return (e2, n = {}) => {
    let { t: a, palette: o } = T(e2, n), r = j(o, n.expression), s = n.size ? ` width="${n.size}" height="${n.size}"` : "", c = w(t.layout(a), n), i2 = Ft(n) + jt(L(t, n, r)) + it(t.render(c.l, r), c.wrap);
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"${s}>${i2}</svg>`;
  };
}
var mt = (t, e2, n) => {
  let a = e2.rx, o = t.num("eye.rx", 0.075, 0.105) * a, r = t.num("eye.ratio", 1.9, 3.2), s = t.num("eye.scale", 0.78, 1.24), c = t.num("eye.stretch", 0.85, 1.18), i2 = t.num("eye.gap", 0.1, 0.24) * a, l = o * Math.max(1, s), m = o * r * Math.max(1, s * c), u2 = l + a * 0.03 + i2, b2 = t.jitter("gaze.x", 0.09) * n.rx, p = t.num("gaze.y", -0.2, 0.08) * n.ry, d2 = t.jitter("eye.dy", 0.04) * n.ry, h = Math.hypot(l, m), f = Math.hypot((Math.abs(b2) + u2 + h) / n.rx, (Math.abs(p) + Math.abs(d2) + h) / n.ry), x2 = f > 0.9 ? 0.9 / f : 1, g = o * x2, k = g * r, C = u2 * x2, Tt = Math.max(0, Math.min(1, i2 / m)), Bt = Math.min(12, Math.asin(Tt) * 180 / Math.PI), H = t.num("eye.lean", -1, 1) * Bt, Lt = Math.max(-12, Math.min(12, H + t.jitter("eye.lean2", 3.5))), X = n.cx + b2 * x2, Y = n.cy + p * x2;
  return [{ cx: X - C, cy: Y, rx: g, ry: k, n: t.num("eye.n", 3.5, 6), rot: H }, { cx: X + C, cy: Y + d2 * x2, rx: g * s, ry: k * s * c, n: t.num("eye.n", 3.5, 6), rot: Lt }];
};
function pt(t, e2) {
  let n = (r) => (t.find(([, s]) => r < s) ?? t[t.length - 1])[0];
  function a(r) {
    let s = n(r("shape")), c = r.num("body.r", 31, 38) * s.core, i2 = { cx: 50 + r.jitter("body.x", 1.5), cy: 50 + r.jitter("body.y", 1.5), rx: c, ry: c * r.num("body.ratio", 0.92, 1.08), n: r.num("body.n", 1.9, 2.5), rot: 0, radii: Array.from({ length: r.int("body.pts", 6, 8) }, (u2, b2) => 1 + r.jitter(`body.r${b2}`, 0.16)) };
    s.body?.(r, i2);
    let l = s.face?.(i2) ?? i2, m = { petals: [], extra: [] };
    return s.decorate?.(r, i2, m), { shape: s.name, draw: s.path, body: i2, face: l, petals: m.petals, extra: m.extra, eyes: e2(r, i2, l) };
  }
  function o(r, s, c) {
    let i2 = (u2) => Math.round(u2 * 100) / 100, l = (u2, b2) => {
      let p = `<path d="${P(u2)}"/>`;
      return c ? `<g class="mo-eye" style="--mo-wrap:${b2 ? 1 : -1};--mo-lean:${i2(u2.rot)};transform-origin:${i2(u2.cx)}px ${i2(u2.cy)}px">${p}</g>` : p;
    }, m = `<g fill="${s.head}">` + r.petals.map((u2) => `<circle cx="${i2(u2.cx)}" cy="${i2(u2.cy)}" r="${i2(u2.r)}"/>`).join("") + r.extra.map((u2) => `<path d="${u2}"/>`).join("") + `<path d="${r.draw ? r.draw(r.body) : P(r.body)}"/></g><g fill="${s.eye}"${c ? ' class="mo-eyes"' : ""}>` + r.eyes.map(l).join("") + "</g>";
...
var z = (t) => (e2) => ({ cx: e2.cx, cy: e2.cy, rx: e2.rx * t, ry: e2.ry * t });
var xt = (t) => z(Math.min(...t.radii) * 0.95)(t);
var _t = (t) => z(0.84)(t);
var ht = { name: "round", core: 1 };
var dt = { name: "organic", core: 0.98, path: bt, face: xt };
var ft = { name: "boxy", core: 0.86, body: (t, e2) => {
  e2.n = t.num("body.n", 3.4, 6), e2.rot = t.num("body.rot", -20, 20);
} };
var gt = { name: "capsule", core: 1.02, body: (t, e2) => {
  e2.ry *= t.num("capsule.squat", 0.55, 0.68);
}, face: z(0.94), decorate: (t, e2, n) => {
  for (let a of [-1, 1]) n.petals.push({ cx: e2.cx + a * (e2.rx - e2.ry), cy: e2.cy, r: e2.ry });
}, path: (t) => rt(t.cx, t.cy, t.rx - t.ry, t.ry) };
var Mt = { name: "nub", core: 0.88, decorate: (t, e2, n) => {
  let a = t.int("nub.n", 1, 2);
  for (let o = 0; o < a; o++) {
    let r = t.num(`nub.a${o}`, 0, 2 * Math.PI);
    n.petals.push({ cx: e2.cx + Math.cos(r) * e2.rx * 0.88, cy: e2.cy + Math.sin(r) * e2.rx * 0.88, r: e2.rx * t.num(`nub.r${o}`, 0.24, 0.4) });
  }
} };
var kt = { name: "cloud", core: 0.78, face: xt, path: bt, decorate: (t, e2, n) => {
  let a = t.int("cloud.n", 4, 6);
  for (let o = 0; o < a; o++) {
    let r = Math.PI + Math.PI * (o + 0.5) / a;
    n.petals.push({ cx: e2.cx + Math.cos(r) * e2.rx * 0.8, cy: e2.cy + Math.sin(r) * e2.rx * 0.5, r: e2.rx * t.num(`cloud.r${o}`, 0.44, 0.62) });
  }
} };
var $t = { name: "droplet", core: 0.78, body: (t, e2) => {
  e2.cy += 0.22 * e2.ry, e2.n = 2;
}, face: (t) => ({ cx: t.cx, cy: t.cy + t.ry * 0.05, rx: t.rx * 0.88, ry: t.ry * 0.88 }), decorate: (t, e2, n) => {
  n.extra.push(ot(e2.cx, e2.cy, e2.rx, e2.ry, t.num("droplet.tip", 1.4, 1.65)));
} };
var Pt = { name: "hexagon", core: 1.05, path: yt, face: _t, body: (t, e2) => {
  e2.sides = 6, e2.rot = t.num("body.rot", -12, 12), e2.round = t.num("poly.round", 0.24, 0.5);
} };
var Ot = { name: "sun", core: 0.7, decorate: (t, e2, n) => {
  let a = t.int("sun.n", 6, 9), o = e2.rx * t.num("sun.dist", 1, 1.08), r = e2.rx * t.num("sun.r", 0.2, 0.26), s = t.num("sun.rot", 0, 2 * Math.PI);
  for (let c = 0; c < a; c++) {
    let i2 = s + 2 * Math.PI * c / a;
    n.petals.push({ cx: e2.cx + Math.cos(i2) * o, cy: e2.cy + Math.sin(i2) * o, r });
  }
} };
var St = { name: "triangle", core: 1.15, path: yt, body: (t, e2) => {
  e2.sides = 3, e2.rot = t.num("body.rot", -5, 5), e2.round = t.num("poly.round", 0.24, 0.5);
...
function i(t, e2) {
  return { l: { ...t, eyes: t.eyes.map((s, n) => ({ ...s, cx: s.cx + e2.edx * (n ? 1 : -1), cy: s.cy + e2.edy + (n ? e2.edy2 : 0), rx: s.rx * (e2.esx + (n ? e2.esx2 : 0)), ry: s.ry * (e2.esy + (n ? e2.esy2 : 0)), rot: s.rot * (1 - e2.lock) + (e2.tilt + (n ? e2.tilt2 : 0)) * (n ? 1 : -1) })) }, wrap: e2.bdy !== 0 ? `translate(0 ${u(e2.bdy)})` : "" };
}
function x(t) {
  let e2 = {};
  for (let s in b) {
...
var MOOD_EXPRESSION = {
  ...
  thinking: ue,
  acting: ee,
  // a live run: the face is quietly pleased, not neutral
  gate: ae,
  sealed: ce,
  refused: te
};
...
var DEFAULT_GENERALIST_NAME = "Chief Steward";

// probe/face.test.ts
var ROOT = ".".length > 0 ? "." : process.cwd();
var MOODS = ["idle", "thinking", "acting", "gate", "sealed", "refused"];
describe("the Generalist's one face", () => {
  it2("is a pure function of the name \u2014 same name, same face", () => {
    assert.equal(Ct("Chief Steward"), Ct("Chief Steward"));
    assert.equal(Ct(DEFAULT_GENERALIST_NAME).slice(0, 4), "<svg");
  });
  it2("different names render different faces", () => {
    assert.notEqual(Ct("Chief Steward"), Ct("My Harbor"));
  });
...
describe("the old OSS engine is gone", () => {
  it2("no vendored oneworks directory survives", () => {
    assert.ok(!fs.existsSync(path.join(ROOT, "src", "vendor", "oneworks-avatar")));
  });
  it2("no source file imports or names OneWorks", () => {
    const walk = (d2) => fs.readdirSync(d2, { withFileTypes: true }).flatMap((e2) => e2.isDirectory() ? walk(path.join(d2, e2.name)) : [path.join(d2, e2.name)]);
...
describe("the console is wired", () => {
  ...
  it2("the console renders evidence, not vibes \u2014 gate banner, digest chips, plan-only honesty", () => {
