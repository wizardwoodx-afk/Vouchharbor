...
function listHandoffs() {
  const raw = storage2()?.getItem(HANDOFFS_KEY) ?? null;
  if (!raw) return [];
  try {
...
function load() {
  ...
  try {
    const p = JSON.parse(s.getItem(KEY3) ?? "");
...
function save(st) {
  ...
  if (s) {
    try {
      s.setItem(KEY3, JSON.stringify(st));
      return;
    } catch {
    }
  }
...
ok(
  "the store imports the memory, vault, handoff, initiative and face surfaces",
...
ok("no plaintext provider key path survives anywhere in src/ui", !fs.readdirSync(path.join(ROOT, "src/ui"), { recursive: true }).some((f3) => {
  const abs = path.join(ROOT, "src/ui", String(f3));
...
ok("the fleet count is SELF-PROVING: catalogStats().byProvenance sums to the count (460 seed + 160 broader + 140 reach + 390 matured + 350 finance + 350 silicon = 1,850)", stats.count === 1850 && ["seed", "broader", "reach", "matured", "finance", "silicon"].map((k) => stats.byProvenance[k] ?? 0).reduce((a, b2) => a + b2, 0) === stats.count && stats.byProvenance.financeIn + stats.byProvenance.financeIntl === stats.byProvenance.finance, JSON.stringify(stats.byProvenance));
