...
// src/vh19/bew.ts
var BEW_PHASES = ["intake", "plan", "act", "verify", "recover", "report"];
...
function hasBew(prompt) {
  return prompt.includes("### Skill: Behaviour Enforcement Workflow [BEW]") && BEW_PHASES.every((p) => prompt.toLowerCase().includes(p));
}
...
var skillsSrc = fs.readFileSync(path.join(ROOT, "src/vh19/skills.ts"), "utf8");
var bewSrc = fs.readFileSync(path.join(ROOT, "src/vh19/bew.ts"), "utf8");
function main() {
  ok("six phases named in order", JSON.stringify(BEW_PHASES) === JSON.stringify(["intake", "plan", "act", "verify", "recover", "report"]));
  ok(
    "four error classes with matching recoveries",
    BEW_ERROR_CLASSES.length === 4 && BEW_ERROR_CLASSES.map((e) => e.cls).join(",") === "transient,bad-input,blocked,gate" && BEW_ERROR_CLASSES.every((e) => e.recovery.length > 20)
  );
  ok(
    "gate rung never impersonates approval",
  ...
  ok(
    "BEW block carries header + checklist + all phases",
  ...
  ok(
    "BEW rides a sample across the fleet (seed, middle, last)",
  ...
  ok(
    "BEW comes AFTER doctrine and skills in the composed prompt",
    buildSpecialistPrompt(fin).indexOf("Operator doctrine") < buildSpecialistPrompt(fin).indexOf("Behaviour Enforcement Workflow") && (skillsFor(fin).length === 0 || buildSpecialistPrompt(fin).indexOf("### Skill: Reconciliation Proof") < buildSpecialistPrompt(fin).indexOf("Behaviour Enforcement Workflow"))
  );
  ok(
    "buildSpecialistPrompt source appends BEW_BLOCK unconditionally",
    skillsSrc.includes("${OPERATOR_DOCTRINE}\\n\\n${BEW_BLOCK}") && skillsSrc.includes('import { BEW_BLOCK } from "./bew"')
  );
  ok(
    "BEW module obeys its own condenser-survival format (### header + Checklist line)",
    /^### Skill: Behaviour Enforcement Workflow \[BEW\]\nChecklist: intake · plan · act · verify · recover · report/m.test(BEW_BLOCK)
  );
  ok(
    "no fake phase names \u2014 phases appear verbatim in the block",
  ...
  ok(
    "an ordered run reports done with a clean trail",
  ...
  skip.to("plan");
  skip.to("act");
  const skipReceipt = skip.finish("done");
  ok(
    "a run that never verified CANNOT claim done \u2014 the machine downgrades to partial and records why",
  ...
  early.to("report");
  ok(
    "report-without-verify is recorded as a violation (and still cannot claim done)",
  ...
  twice.to("recover");
  ok(
    "a second recovery in one run is a recorded violation (the ladder allows ONE situation-changing retry)",
  ...
  ok(
    "runMemberAgent creates a BewRun and records plan/act/verify/recover",
  ...
  ok(
    "truncated loops finish partial \u2014 a step-limited run cannot claim done",
  ...
  ok(
    "the generalist surfaces the BEW receipt on member sections and rides it into member digests",
