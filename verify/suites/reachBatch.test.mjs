...
test("reach batch \u2014 200 industry specialists that do not disturb the fleet", async (t) => {
  await t.test("\xA71 the snapshot is exactly what the spec builds", () => {
  ...
  await t.test("\xA72 forty domains, five stations, and the census the spec states", () => {
  ...
  await t.test("\xA73 disjoint from every bench the fleet already ships", () => {
  ...
  await t.test("\xA74 every entry is a specified specialist, not a placeholder", () => {
    for (const e of REACH_BATCH_SPECIALISTS) {
      assert.equal(e.provenance, REACH_BATCH_PROVENANCE);
      assert.equal(e.riskTier, STATION_RISK[e.id.split(".").pop()], `${e.id} carries the wrong tier for its station`);
      ...
      if (domain) {
        ...
        assert.equal(e.id, batchEntryId(domain, station), `${e.id} is not the id the spec builds`);
      }
    }
  });
  await t.test("\xA75 the batch is additive: the registry's own numbers do not move", () => {
