...
function clearMemory(userId = "default") {
  ...
  saveAll(all.filter((r2) => r2.userId !== userId));
}
...
function gradeExam(sessionId, grades, now = () => /* @__PURE__ */ new Date()) {
  ...
  saveGrant(loadGrant(session2.userId, session2.category ?? void 0).attempts + 1, passed ? score : null, passed, session2.userId, now, session2.category ?? void 0);
...
function recordUsage(entry, now = () => /* @__PURE__ */ new Date()) {
  const raw = storage7()?.getItem(LEDGER_KEY);
  let list = [];
  try {
  ...
  storage7()?.setItem(LEDGER_KEY, JSON.stringify(list.slice(-LEDGER_CAP)));
}
...
function scriptedFetch(responder) {
  ...
  const impl = (async (input, init) => {
    ...
    const r2 = responder(calls[calls.length - 1]);
    return new Response(JSON.stringify(r2.body), { status: r2.status });
  });
  return { impl, calls };
}
...
test("vh19 \u2014 registry, router, providers, memory, exam, generalist", async () => {
  ...
  assert.ok(exam.ok);
  check("exam questions cite REAL source records", exam.session.questions.every((q) => loadMemory("probe-user").some((r5) => r5.id === q.sourceRecordId)));
  ...
  check("wrong answers became correction records in memory (feedbackLearned)", g9.ok && g9.feedbackLearned === 1 && loadMemory("probe-user").some((r5) => r5.kind === "correction" && r5.reason === "should have asked first"));
  resetExams("probe-user");
  ...
  check("a category-scoped exam proposes from that category only", secExam.ok === true && secExam.ok === true && secExam.session.category === "security" && secExam.session.questions.every((q) => loadMemory("cat-user").find((r5) => r5.id === q.sourceRecordId)?.category === "security"));
