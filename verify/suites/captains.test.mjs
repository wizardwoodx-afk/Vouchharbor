...
function planDomainWork(captainId, task, cap = 3) {
  ...
  })).filter((m) => m.score > 0).sort((a, b2) => b2.score - a.score || a.specialistId.localeCompare(b2.specialistId)).slice(0, cap);
}
...
function clearTokenLedger() {
  storage5()?.removeItem(LEDGER_KEY);
}
...
test("captains + failures \u2014 oversight that never fabricates", async () => {
  ...
  check("the run ends in the captain's OWN synthesis (19.3.0)", multi.synthesis !== void 0 && multi.reply.includes("CAPTAIN-SYNTHESIS") && /^[0-9a-f]{64}$/.test(multi.synthesis?.digest ?? ""), multi.synthesis?.digest);
  callNo = 0;
  calls.length = 0;
  failCallNo = 2;
