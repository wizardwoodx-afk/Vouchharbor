...
test("specialist tools \u2014 real execution, gated and receipted (19.3.0)", async () => {
  ...
  let g = 0;
  const gateFetch = (async () => new Response(JSON.stringify({ choices: [{ message: { content: gateScript[Math.min(g++, gateScript.length - 1)] } }] }), { status: 200 }));
  let gateAsks = 0;
  const deniedRun = await runMemberAgent({
    ...
    systemBase: "You are the typescript specialist.",
    fetchImpl: gateFetch,
    toolCtx: { workspaceRoot: root, gate: async () => {
      gateAsks++;
      return { approved: false, reason: "policy: no overwrite in probes" };
    } },
    hash: hasher
  });
  ...
  const toolless = await runMemberAgent({
    ...
    systemBase: "You are the typescript specialist.",
    fetchImpl: (async () => new Response(JSON.stringify({ choices: [{ message: { content: "Generics parameterize types." } }] }), { status: 200 })),
    hash: hasher
  });
  ...
  const doorFetch = (async (_input, init) => {
