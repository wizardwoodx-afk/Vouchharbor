...
var require_dynamicAnchor = __commonJS({
  "node_modules/ajv/dist/vocabularies/dynamic/dynamicAnchor.js"(exports) {
    ...
    function dynamicAnchor(cxt, anchor) {
      ...
      const validate = it2.errSchemaPath === "#" ? it2.validateName : _getValidate(cxt);
    ...
    var def = {
      ...
      code({ keyword, parentSchema, it: it2 }) {
        if (parentSchema.contains === void 0) {
          (0, util_1.checkStrictMode)(it2, `"${keyword}" without "contains" is ignored`);
        }
      }
    };
    exports.default = def;
...
var FIX = path.join(ROOT, "tools", "mcp-conformance", "fixtures");
var FIXTURES = {
...
var request = (method, params) => send(method, params).reply;
var sleep = (ms) => new Promise((r) => setTimeout(r, ms));
var ajv2026 = new import__.default({ strict: false, validateFormats: false });
var ajvTasks = new import__.default({ strict: false, validateFormats: false });
var ajv2025 = new import_ajv.default({ strict: false, validateFormats: false });
...
function validator(ajv, doc, def) {
...
function conforms(ajv, doc, def, data, label) {
...
function conformsRequest(ajv, doc, def, req, label) {
  const v = validator(ajv, doc, def);
  const ok = v(req);
...
describe("C3 legacy era \u2014 2025-06-18 clients see exactly their era's wire", () => {
  ...
  it("legacy gate flow conforms: no resultType, approval via approve_action + call_status, receipt verifies", async () => {
    ...
    const text = first.result.content[0].text;
