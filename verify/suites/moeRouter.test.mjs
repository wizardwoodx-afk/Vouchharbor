...
// src/moe/catalog.gen.ts
var SPECIALIST_CATALOG = [
  {
    "id": "ab-test-analysis",
...
var DEFAULT_CONFIDENCE_FLOOR = 0.18;
var RECENCY_HALF_LIFE = 8;
...
function tokenize(text) {
  return String(text).toLowerCase().replace(/[^a-z0-9+#.]+/g, " ").split(" ").map((w2) => w2.replace(/^\.+|\.+$/g, "")).filter((w2) => w2.length > 2 && w2.length < 25 && !STOP.has(w2) && !/^\d+$/.test(w2));
}
function capabilityScore(taskTokens, keywords) {
...
function learnedPriorOf(accepted2, rejected2) {
  const total = accepted2 + rejected2;
  if (total === 0) return 0;
  const rate = accepted2 / total;
  const confidence = 1 - Math.exp(-total / 5);
  return (rate - 0.5) * 2 * confidence;
}
function recentSuccessOf(history) {
  let num = 0;
  let den = 0;
  history.slice(-24).forEach((v, i, arr) => {
    const age = arr.length - 1 - i;
    const w2 = Math.pow(0.5, age / RECENCY_HALF_LIFE);
    num += v === "correct" ? w2 : 0;
    den += w2;
  });
...
function scoreAll(input) {
  ...
  const scores = input.catalog.map((s) => {
    const capability = capabilityScore(taskTokens, s.keywords);
    const w2 = input.weights.get(s.id);
    const learnedPrior = w2 ? w2.learnedPrior : 0;
    const recentSuccess = w2 ? w2.recentSuccess : 0.5;
    const score = capability * ROUTER_WEIGHTS.capability + learnedPrior * ROUTER_WEIGHTS.learnedPrior + (recentSuccess - 0.5) * ROUTER_WEIGHTS.recentSuccess;
    return {
      specialistId: s.id,
  ...
  scores.sort((a2, b2) => b2.score - a2.score || a2.specialistId.localeCompare(b2.specialistId));
  return scores;
}
function route(input) {
  ...
  const unservable = chosen ? [...new Set(input.task.requestedTools)].filter((t) => !host.has(t)) : [];
  const ceilingCheck = {
    ...
    exceeded: [.../* @__PURE__ */ new Set([...exceeded, ...unservable])].sort()
  };
  let outcome;
  if (!chosen) {
    outcome = {
...
function applyVerdict(weights, specialistId, judgement, historyFor, at = (/* @__PURE__ */ new Date()).toISOString()) {
  ...
  const accepted2 = prev.accepted + (judgement === "correct" ? 1 : 0);
  const rejected2 = prev.rejected + (judgement === "wrong" ? 1 : 0);
  next.set(specialistId, {
    specialistId,
    accepted: accepted2,
    rejected: rejected2,
    learnedPrior: Number(learnedPriorOf(accepted2, rejected2).toFixed(6)),
    recentSuccess: Number(recentSuccessOf([...historyFor(specialistId), judgement]).toFixed(6)),
    updatedAt: at
  });
  return next;
}
var TRUST_LADDER = ["shadow", "assist", "supervised", "autonomous"];
...
var HOST = ["Read", "Grep", "Glob", "Bash", "Write", "Edit"];
var weights0 = /* @__PURE__ */ new Map();
...
var task = { text: "add strict input validation to the login form in typescript and cover it with unit tests", requestedTools: ["Read", "Grep", "Write"], assertedTier: "safe" };
var a = route({ catalog: SPECIALIST_CATALOG, weights: weights0, task, trustTier: "supervised", hostToolInventory: HOST, taskId: "t-a" });
var b = route({ catalog: SPECIALIST_CATALOG, weights: weights0, task, trustTier: "supervised", hostToolInventory: HOST, taskId: "t-b" });
ok(
  "two runs pick the same specialist",
...
ok("the margin is top minus second", a.decision.candidates.length < 2 || Math.abs(a.decision.margin - (a.decision.candidates[0].score - a.decision.candidates[1].score)) < 1e-6, String(a.decision.margin));
section("2. the tool ceiling is a wall, not a hint");
var needsExtra = route({
  catalog: SPECIALIST_CATALOG,
  weights: weights0,
  trustTier: "supervised",
  hostToolInventory: HOST,
...
ok(
  ...
  needsExtra.decision.outcome.kind === "refused" && needsExtra.decision.outcome.reason.includes("chrome-mcp"),
...
var empty = route({ catalog: [], weights: weights0, task, trustTier: "supervised", hostToolInventory: [] });
...
var chosenId = a.decision.outcome.kind === "routed" ? a.decision.outcome.specialistId : null;
ok("baseline routed to a real catalog entry", !!chosenId && SPECIALIST_CATALOG.some((s) => s.id === chosenId), String(chosenId));
var rejected = applyVerdict(weights0, chosenId, "wrong", () => []);
var accepted = applyVerdict(weights0, chosenId, "correct", () => []);
...
var after = route({ catalog: SPECIALIST_CATALOG, weights: w, task, trustTier: "supervised", hostToolInventory: HOST, taskId: "t-after" });
var rankBefore = a.decision.candidates.findIndex((c) => c.specialistId === chosenId);
var rankAfter = after.decision.candidates.findIndex((c) => c.specialistId === chosenId);
ok(`12 rejections demoted the specialist in the ranking (rank ${rankBefore} -> ${rankAfter})`, rankAfter > rankBefore, `chosen=${chosenId}`);
ok(
  "the specialist that got rejected is now penalised in its own score",
...
var vague = route({ catalog: SPECIALIST_CATALOG, weights: weights0, task: { text: "do the thing", requestedTools: [] }, trustTier: "supervised", hostToolInventory: HOST });
...
ok(
  "below the floor the reason names the human gate",
  vague.decision.outcome.kind === "refused" ? /human gate|below floor/i.test(vague.decision.outcome.reason) || /ceiling/.test(vague.decision.outcome.reason) : true,
  vague.decision.outcome.kind === "refused" ? vague.decision.outcome.reason : "routed"
);
section("5. shadow tier computes but does not act");
var shadow = route({ catalog: SPECIALIST_CATALOG, weights: weights0, task, trustTier: "shadow", hostToolInventory: HOST });
...
ok("tokenize survives empty and unicode input", Array.isArray(tokenize("")) && Array.isArray(tokenize("\u65E5\u672C\u8A9E \u{1F389}")));
console.log(`
