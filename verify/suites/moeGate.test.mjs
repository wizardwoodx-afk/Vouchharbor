...
// src/moe/gate.ts
var DEFAULT_THRESHOLD_PERCENT = 90;
var MIN_EXAM_ITEMS = 8;
var DEFAULT_POLICY = {
  thresholdPercent: DEFAULT_THRESHOLD_PERCENT,
  minItems: MIN_EXAM_ITEMS,
  gradingMode: "agent-authored"
};
function scoreExam(items, thresholdPercent) {
...
function evaluateGate(examId, items, trustTierAtAttempt, policy = {}, at = (/* @__PURE__ */ new Date()).toISOString()) {
  const p = { ...DEFAULT_POLICY, ...policy };
  const ladder = ["shadow", "assist", "supervised", "autonomous"];
  const idx = Math.max(0, ladder.indexOf(trustTierAtAttempt));
  const receipt = (prefix, percent2, passedCount2, unansweredCount, caveat2 = "") => `${prefix} | score=${percent2}% | threshold=${p.thresholdPercent}% | items=${passedCount2}/${items.length} | unanswered=${unansweredCount} | mode=${p.gradingMode}${caveat2}`;
  const AGREEMENT_CAVEAT = " | EVIDENCE=USER-AGREEMENT-NOT-INDEPENDENT-COMPETENCE";
  const fail2 = (reason) => {
    const { percent: percent2, passedCount: passedCount2 } = scoreExam(items, p.thresholdPercent);
    ...
    return {
      ...
      receiptNote: receipt(`escalation-refused: ${reason}`, percent2, passedCount2, unansweredCount)
    };
  };
  if (items.length < p.minItems) {
    return fail2(`the exam had ${items.length} item(s), below the minimum of ${p.minItems} \u2014 a percentage from a handful of questions is not evidence`);
  }
  ...
  if (p.gradingMode === "grounded") {
    ...
    if (!g || !g.repoTestsPassed || !g.crossVendorReviewPassed) {
      return fail2("grounded mode requires the repository's own tests AND a different-harness review to pass, and one of those is missing");
    }
  }
  if (!passedRaw) {
    return fail2(`scored ${percent}% against a ${p.thresholdPercent}% threshold (${passedCount}/${items.length}${unanswered ? `, ${unanswered} unanswered and counted as misses` : ""})`);
  }
  const next = ladder[Math.min(idx + 1, ladder.length - 1)];
  const toAutonomy = next === "autonomous";
  const caveat = p.gradingMode === "agent-authored" ? " Grading mode was agent-authored: VH-19 wrote both the scenarios and the answers, so this measures agreement with its model of your work, NOT independently verified competence." : " Grading mode was grounded against the repository's own tests and a cross-vendor review.";
  return {
    result: {
      id: examId,
      trustTierAtAttempt,
    ...
    nextTrustTier: next,
    explanation: toAutonomy ? `Scored ${percent}% (${passedCount}/${items.length}). VH-19 is now requesting FULL AUTONOMOUS operation with human monitoring and a modify option \u2014 this is the approval you asked to be asked for.${caveat}` : `Scored ${percent}% (${passedCount}/${items.length}). Advancing ${trustTierAtAttempt} \u2192 ${next}.${caveat}`,
    receiptNote: receipt(
...
function proposeScenarios(catalog, taskText, tokens, limit = MIN_EXAM_ITEMS) {
  ...
  })).filter((c) => c.fit > 0).sort((a, b) => b.fit - a.fit || a.specialistId.localeCompare(b.specialistId)).slice(0, limit).map(({ specialistId, scenario }) => ({ specialistId, scenario }));
}
...
function tokenize(text) {
  return String(text).toLowerCase().replace(/[^a-z0-9+#.]+/g, " ").split(" ").map((w) => w.replace(/^\.+|\.+$/g, "")).filter((w) => w.length > 2 && w.length < 25 && !STOP.has(w) && !/^\d+$/.test(w));
}

// probe/moeGate.test.ts
var passed = 0;
var failures = [];
...
var mk = (n, verdicts = []) => Array.from({ length: n }, (_, i) => ({
  id: `q${i}`,
  scenario: `scenario ${i}`,
...
ok("rounding cannot smuggle a pass \u2014 90% of 10 means 9/10 exactly", (() => {
  const r9 = evaluateGate("e9", mk(10, new Array(9).fill("correct").concat(["wrong"])), "supervised");
  return r9.result.passed === true && r9.result.scorePercent === 90;
})());
ok("8.99 of 10 is not 9", (() => {
  const r = scoreExam(mk(10, new Array(8).fill("correct").concat(["wrong", "wrong"])), 90);
  return r.percent === 80;
})());
...
ok("unanswered items count as misses, not skips", (() => {
  const r = evaluateGate("e3", mk(10, new Array(7).fill("correct").concat(["unanswered", "unanswered", "unanswered"])), "supervised");
  return r.result.passed === false;
})());
ok("an unanswered-heavy exam says so in the receipt", (() => {
  const r = evaluateGate("e4", mk(10, new Array(6).fill("correct").concat(["unanswered", "unanswered", "wrong", "wrong"])), "supervised");
...
var pass = evaluateGate("e6", mk(10), "supervised");
...
var fail = evaluateGate("e8", mk(10, new Array(5).fill("correct").concat(new Array(5).fill("wrong"))), "autonomous");
ok("a fail drops the tier all the way back to shadow", fail.nextTrustTier === "shadow" && fail.result.passed === false);
ok(
  "the receipt records the score, threshold, item counts and mode",
...
ok(
  "agent-authored pass states it measures agreement, not competence",
...
ok("the graded-by and provenance fields are fixed, not caller-supplied", (() => {
  const v = { taskId: "t", specialistId: "x", judgement: "correct", agentClaim: "c", gradedBy: "user", answerProvenance: "agent-authored", at: "now" };
...
var proposed = proposeScenarios(
  SPECIALIST_CATALOG.map((s) => ({ id: s.id, title: s.title, description: s.description, keywords: s.keywords })),
  "add strict input validation to the login form in typescript and cover it with unit tests",
  tokenize,
  8
);
