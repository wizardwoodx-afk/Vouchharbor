...
async function loadOrCreate(storage13, identity, security) {
  const owner = identity.trim() || AUTHORITY_OWNER_FALLBACK;
  if (storage13) {
    try {
      const raw = storage13.get();
      if (raw) {
  ...
  const ident = { keys, owner, security, persisted: Boolean(storage13) };
  if (storage13) {
    try {
      ...
      storage13.set(JSON.stringify({ byOwner }));
    } catch {
    }
  }
  return ident;
...
async function authorityOwnerIdentity(opts) {
  ...
  let { storage: storage13, security } = await defaultStorageCache.promise;
  let unlockFailed = false;
  if (storage13 && storage13.sealed) {
    storage13 = null;
    security = "session";
    unlockFailed = true;
  }
  ...
  let m = realmCache.get(storage13);
  if (!m) {
    m = /* @__PURE__ */ new Map();
    realmCache.set(storage13, m);
  }
  const hit = m.get(owner);
  if (hit) return hit;
  const ident = await loadOrCreate(storage13, owner, security);
  m.set(owner, ident);
  return ident;
}
...
function saveAll(records) {
  ...
  s.setItem(KEY4, JSON.stringify(capped));
}
function recordDecision(input) {
  const rec = { id: uid("dec"), ts: input.ts ?? nowIso(), ...input };
  const s = storage6();
  const all = s ? JSON.parse(s.getItem(KEY4) ?? "[]") : [];
  all.push(rec);
  saveAll(all);
  return rec;
}
...
async function askVH19(args, deps = {}) {
  ...
  const finish = async (r2) => {
    ...
    if (!mandate) return { ...full, authority: null, provenanceDigest: await sha256Hex3(responseCanonical({ ...full, authority: null })) };
    const mandateDigest = await sha256Hex3(mandateCanonical(mandate));
    ...
    const provenanceDigest = await sha256Hex3(responseCanonical(full2));
...
function listHandoffs() {
  const raw = storage9()?.getItem(HANDOFFS_KEY) ?? null;
  if (!raw) return [];
  try {
...
function listByoaAgents() {
  ...
  try {
    return JSON.parse(s.getItem(KEY7) ?? "[]");
  } catch {
    return session4;
  }
}
function persist(all) {
  ...
  if (s) {
    try {
      s.setItem(KEY7, JSON.stringify(all));
      return;
    } catch {
    }
  }
...
async function sha256Hex5(text) {
  const buf = await globalThis.crypto.subtle.digest("SHA-256", new TextEncoder().encode(text));
  return Array.from(new Uint8Array(buf)).map((x) => x.toString(16).padStart(2, "0")).join("");
}
function byoaDelegate(agent, opts = {}) {
  return async (d) => {
    ...
    if (!trust.ok) {
      ...
      const digest = await sha256Hex5(JSON.stringify({ peer: agent.id, task: d.task, ok: false, detail: detail2, at }));
    ...
    if (gate) {
      ...
      if (!decision.approved) {
        const digest = await sha256Hex5(JSON.stringify({ peer: agent.id, task: d.task, ok: false, detail: `denied at the gate: ${decision.reason ?? "no reason given"}`, at }));
    ...
    const receiptDigest = await sha256Hex5(JSON.stringify({ token: "vh.byoa.delegation.v1", peer: agent.id, identity: agent.identityDigest ?? "", ceiling: agent.ceiling, task: d.task, ok: ok2, detail: detail.slice(0, 400), findings: findings ?? [], at }));
...
var ok = (name, cond, detail) => {
  if (cond) {
    ...
    failed++;
    failures.push(name);
    console.log(`  FAIL ${name}${detail !== void 0 ? ` \u2014 ${JSON.stringify(detail).slice(0, 160)}` : ""}`);
  }
};
var section = (t) => console.log(`
\u2500\u2500 ${t}`);
var t0 = Date.now();
var scriptedProvider = { kind: "openai-compatible", baseUrl: "https://api.openai.com/v1", apiKey: "sk-agentic-test", model: "test-model" };
var scriptedFetch = (answer) => (async (input) => {
...
async function main() {
  section("A \xB7 ROUTING \u2014 the Generalist fields real tasks");
  const tasks = [
  ...
  let routedCount = 0;
  const routedCategories = /* @__PURE__ */ new Set();
  let digestCount = 0;
  for (const text of tasks) {
    ...
    if (resp.routed.selected.length > 0) {
      routedCount++;
      resp.routed.selected.forEach((s) => routedCategories.add(getSpecialist(s.id)?.category ?? "?"));
    }
  ...
  section("B \xB7 GOVERNANCE \u2014 the gate is in the run");
  ...
  section("C \xB7 AUTONOMY \u2014 the exam is real");
  for (let i = 0; i < 12; i++) {
  ...
  let examScore = 0;
  if (ex.ok) {
    const grades = ex.session.questions.map((q) => ({ questionId: q.id, verdict: "correct" }));
  ...
  section("D \xB7 RSIRALS \u2014 the full loop on the real engine");
  ...
  section("E \xB7 BYOA \u2014 the security battery");
  let ssrfBlocked = false;
  try {
    registerByoaAgent({ name: "ssrf", kind: "openai-compatible", endpoint: "http://169.254.169.254/latest", ceiling: "safe", capabilities: [] });
  } catch {
    ssrfBlocked = true;
  }
  ...
  try {
    registerByoaAgent({ name: "plainhttp", kind: "openai-compatible", endpoint: "http://agent.example.com/v1", ceiling: "safe", capabilities: [] });
  } catch {
    tlsBlocked = true;
  }
  ...
  const tampered = { ...goodAgent, endpoint: "https://evil.example.com/v1" };
  ...
  let rateHits = 0;
  for (let i = 0; i < 12; i++) {
    if (!byoaRateGate.check("byoa.flood.agentic")) rateHits++;
  }
  ok("the rate ceiling stops a delegation flood", rateHits >= 2);
  section("F \xB7 CATALOG \u2014 self-proving composition");
  const stats = catalogStats();
  ok("1,150 registered specialists = 460 seed + 160 broader + 140 reach + 390 matured", stats.count === 1150 && stats.byProvenance.seed === 460 && stats.byProvenance.broader === 160 && stats.byProvenance.reach === 140 && stats.byProvenance.matured === 390);
  ...
  console.log(`
${"\u2550".repeat(56)}
AGENTIC TEST PROTOCOL v1 \u2014 ${passed} passed, ${failed} failed \xB7 ${secs}s \xB7 governance plane T_v${GOVERNANCE_PLANE.version}
${"\u2550".repeat(56)}`);
