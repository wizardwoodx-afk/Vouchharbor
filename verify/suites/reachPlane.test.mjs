...
function missionBrowser(missionId, transport, binary, spawn) {
  ...
  if (!b2) {
    b2 = new HeadlessBrowser(newProfile(missionId), transport ?? fetchTransport, binary ?? null, spawn);
    missionBrowsers.set(missionId, b2);
  } else {
    if (transport && b2.transport !== transport) b2.transport = transport;
    if (binary !== void 0 && binary !== null && b2.binary !== binary) b2.binary = binary;
    if (spawn && b2.spawn !== spawn) b2.spawn = spawn;
  }
  return b2;
}
function endMissionBrowser(missionId) {
  missionBrowsers.delete(missionId);
}
...
var HeadlessBrowser = class {
  ...
  screenshot(outPath) {
    ...
    const r2 = this.spawn(this.binary, ["--headless", "--disable-gpu", "--no-sandbox", `--screenshot=${outPath}`, `--window-size=${this.profile.viewport.width},${this.profile.viewport.height}`, last.result.url]);
    if (r2.status !== 0) {
      return this.record({ kind: "pc.browser", action, decision: "refused", result: last.result, reason: `browser binary exited ${r2.status}: ${(r2.stderr ?? "").slice(0, 200)}` });
    }
...
var sha256HexWeb = async (t) => {
  ...
  return Array.from(new Uint8Array(d)).map((b2) => b2.toString(16).padStart(2, "0")).join("");
};
...
async function loadOrCreate(storage9, identity, security) {
  const owner = identity.trim() || AUTHORITY_OWNER_FALLBACK;
  if (storage9) {
    try {
      const raw = storage9.get();
      if (raw) {
  ...
  const ident = { keys, owner, security, persisted: Boolean(storage9) };
  if (storage9) {
    try {
      ...
      storage9.set(JSON.stringify({ byOwner }));
    } catch {
    }
  }
  return ident;
...
async function authorityOwnerIdentity(opts) {
  ...
  let { storage: storage9, security } = await defaultStorageCache.promise;
  let unlockFailed = false;
  if (storage9 && storage9.sealed) {
    storage9 = null;
    security = "session";
    unlockFailed = true;
  }
  ...
  let m = realmCache.get(storage9);
  if (!m) {
    m = /* @__PURE__ */ new Map();
    realmCache.set(storage9, m);
  }
  const hit = m.get(owner);
  if (hit) return hit;
  const ident = await loadOrCreate(storage9, owner, security);
  m.set(owner, ident);
  return ident;
}
...
async function attestMissionRun(req, opts) {
  const executed = [...new Set(req.executedTools)].filter((t) => ISSUABLE_SCOPE.includes(t)).sort();
  if (executed.length === 0) return null;
  const actions = req.executedTools.filter((t) => ISSUABLE_SCOPE.includes(t)).length;
  const now = opts?.now ?? Date.now();
  const ident = await authorityOwnerIdentity(opts);
  return signMandateWeb({
    agentId: `vh19:${req.missionId}`,
...
async function recordMissionAuthority(missionId, responseDigest, mandate, mandateDigest, publicKeyPem, now = Date.now()) {
...
function missionAuthorityFor(responseDigest) {
  return loadLedger().find((r2) => r2.responseDigest === responseDigest) ?? null;
}
...
async function reachMcpCall(name, args, ctx) {
  ...
  switch (name) {
    case "pc.exec": {
      const r2 = pcExec(String(args.binary ?? ""), Array.isArray(args.args) ? args.args.map(String) : [], policy, "risky", ctx.run ? { run: ctx.run } : {});
      return { ok: r2.decision === "executed", decision: r2.decision, output: r2.decision === "executed" ? `exit ${r2.exitCode} \u2014 ${r2.stdoutPreview || "(no output)"}` : r2.reason, receipt: r2 };
    }
    ...
    case "pc.browser.navigate": {
      ...
      const r2 = await browser.open(String(args.url ?? ""));
      return { ok: r2.decision === "executed", decision: r2.decision, output: r2.decision === "executed" && r2.result ? `loaded ${r2.result.url} (${r2.result.status}) \u2014 ${r2.result.title || "(no title)"}` : r2.reason, receipt: r2 };
    }
    case "pc.browser.screenshot": {
      const browser = missionBrowser(ctx.missionId, ctx.transport ?? void 0, ctx.exists ? detectBrowserBinary(void 0, ctx.exists) : null, ctx.spawn);
      if (typeof args.url === "string" && args.url) await browser.open(args.url);
      const r2 = browser.screenshot(String(args.outPath ?? ""));
      return { ok: r2.decision === "executed", decision: r2.decision, output: r2.reason, receipt: r2 };
    }
    ...
    case "authority.verify": {
      const r2 = await verifyMandateWeb(args.mandate, String(args.publicKeyPem ?? ""));
      return { ok: r2.ok, decision: r2.ok ? "executed" : "refused", output: r2.ok ? "mandate verifies with the public key" : r2.detail };
    }
...
function execPcExec(input, ctx) {
  ...
  const r2 = pcExec(binary, args, ctx.pc.policy, "risky", ctx.pc.run ? { run: ctx.pc.run } : {});
  if (r2.decision === "executed") return { outcome: "ok", output: `exit ${r2.exitCode}${r2.timedOut ? " (timed out)" : ""} \u2014 ${r2.stdoutPreview || "(no output)"} \u2014 ${r2.reason}` };
  if (r2.decision === "handover") return { outcome: "gated-out", output: r2.reason };
  return { outcome: "refused", output: r2.reason };
}
async function execPcBrowser(input, ctx) {
  if (!ctx.pc) return { outcome: "refused", output: "computer-use plane is not attached to this mission \u2014 the Generalist attaches it only for reach missions" };
  const action = input.action;
  const browser = missionBrowser(ctx.pc.missionId, ctx.pc.transport ?? void 0);
  if (action === "open" || action === "navigate") {
    const url = typeof input.url === "string" ? input.url : "";
    if (!url) return { outcome: "error", output: "pc.browser open/navigate needs a url" };
    const r2 = await browser.open(url);
    if (r2.decision === "executed" && r2.result) return { outcome: "ok", output: `loaded ${r2.result.url} (${r2.result.status}) \u2014 title: ${r2.result.title || "(none)"} \u2014 ${r2.result.links.length} links` };
    if (r2.decision === "handover") return { outcome: "gated-out", output: r2.reason };
    return { outcome: "refused", output: r2.reason };
  }
  if (action === "screenshot") {
    ...
    const r2 = browser.screenshot(outPath);
    if (r2.decision === "executed") return { outcome: "ok", output: r2.reason };
    if (r2.decision === "handover") return { outcome: "gated-out", output: r2.reason };
    return { outcome: "refused", output: r2.reason };
  }
...
function memberToolIds(category, toolCtx) {
  ...
  return toolCtx?.pc ? [...base, "pc.exec", "pc.browser"] : base;
}
...
var KEY3 = "vh19.skills.imported.v1";
var session = [];
function storage4() {
  try {
...
function buildCaptainReport(captainId, results) {
  ...
  const failures = results.filter((r2) => r2.outcome !== "answered" && r2.outcome !== "peer-delegated").map((r2) => `${getSpecialist(r2.specialistId)?.name ?? r2.specialistId}: ${r2.outcome}${r2.note ? ` \u2014 ${r2.note.slice(0, 80)}` : ""}`);
...
async function askVH19(args, deps = {}) {
  ...
  const finish = async (r2) => {
    ...
    if (!mandate) return { ...full, authority: null, provenanceDigest: await sha256Hex2(responseCanonical({ ...full, authority: null })) };
    const mandateDigest = await sha256Hex2(mandateCanonical(mandate));
    const ident = await authorityOwnerIdentity({ identity: userId });
    const authority = { mandateDigest, scheme: "ecdsa-p256", owner: mandate.owner };
    const full2 = { ...full, authority };
    const provenanceDigest = await sha256Hex2(responseCanonical(full2));
...
Routing: ${routed.strategy} via ${routed.routedBy} (${routed.selected.length} of ${routed.considered} specialists considered).` + (routed.fallbackReason ? ` Note: ${routed.fallbackReason}.` : ""),
  ...
  const ctxFor = (s) => memberToolCtx && s.provenance === "vh-19.5.1-reach" ? { ...memberToolCtx, pc: reachPc } : memberToolCtx;
...
// probe/reachPlane.test.ts
var sha2562 = (t) => createHash("sha256").update(t).digest("hex");
var GATE_OK = async () => ({ approved: true, reason: "probe approves" });
var GATE_NO = async () => ({ approved: false, reason: "probe declines" });
var nodeishRun = (bin, args) => ({ status: 0, timedOut: false, stdout: `${bin} ${args.join(" ")}`, stderr: "" });
var memStore = () => {
  let v = null;
  return { get: () => v, set: (x) => {
    v = x;
...
test("Agent Reach MCP \u2014 primary default server, governed computer-use", async (t) => {
  await t.test("the server self-describes as PRIMARY + default with six tools", () => {
    const info2 = reachMcpServerInfo();
    assert.equal(info2.name, "Agent Reach MCP");
    assert.equal(info2.primary, true);
    assert.equal(info2.default, true);
    assert.equal(info2.tools.length, 6);
    assert.ok(info2.tools.some((x) => x.name === "pc.exec"));
    assert.ok(info2.tools.some((x) => x.name === "authority.verify"));
  });
  await t.test("risky tools never run without a human gate", async () => {
    const r2 = await reachMcpCall("pc.exec", { binary: "ls" }, { missionId: "m-exec" });
    assert.equal(r2.ok, false);
    assert.equal(r2.decision, "gated-out");
  });
  await t.test("gate denial is recorded as gated-out, nothing executed", async () => {
    const r2 = await reachMcpCall("pc.exec", { binary: "ls" }, { missionId: "m-exec", gate: GATE_NO, run: nodeishRun });
    assert.equal(r2.decision, "gated-out");
    assert.match(r2.output, /declined/);
  });
  await t.test("gate approval + allowlisted binary executes and carries a receipt", async () => {
    const r2 = await reachMcpCall("pc.exec", { binary: "ls", args: ["-la"] }, { missionId: "m-exec", gate: GATE_OK, run: nodeishRun });
    assert.equal(r2.ok, true);
    assert.equal(r2.decision, "executed");
    assert.ok(r2.receipt && r2.receipt.digest.length === 64, "receipt digest present");
  });
  await t.test("a non-allowlisted binary is refused even with the gate open", async () => {
    const r2 = await reachMcpCall("pc.exec", { binary: "rm", args: ["-rf", "/"] }, { missionId: "m-exec", gate: GATE_OK, run: nodeishRun });
    assert.equal(r2.decision, "refused");
    assert.match(r2.output, /allowlist/);
  });
  await t.test("injection payloads inside pc.exec are refused", async () => {
    const r2 = await reachMcpCall("pc.exec", { binary: "echo", args: ["$(whoami)"] }, { missionId: "m-exec", gate: GATE_OK, run: nodeishRun });
    assert.equal(r2.decision, "refused");
    assert.match(r2.output, /injection|metacharacters/i);
  });
  ...
  await t.test("pc.browser.open executes via injected transport", async () => {
    ...
    const fake = {
      open: async (url) => ({ url, status: 200, title: "A", bodyText: "hello", links: [], images: [] }),
      act: async (s) => s
    };
  ...
  await t.test("unknown tool names refuse honestly with the surface list", async () => {
    const r2 = await reachMcpCall("pc.format_disk", {}, { missionId: "m-exec", gate: GATE_OK });
    assert.equal(r2.decision, "refused");
    assert.match(r2.output, /unknown tool/);
  });
});
test("portable authority \u2014 owner-granted, attested runs, durable identity", async (t) => {
  await t.test("issue \u2192 verify round-trip with the public key alone", async () => {
    ...
    const r2 = await verifyMandateWeb(m, ident.keys.publicKeyPem);
    assert.equal(r2.ok, true);
  ...
  await t.test("authority.issue is OWNER-GATED: no gate, nothing issued", async () => {
    const r2 = await reachMcpCall("authority.issue", { missionId: "m-nogate", scope: ["fs.read"] }, { missionId: "m-nogate" });
    assert.equal(r2.decision, "gated-out");
    assert.equal(r2.ok, false);
  });
  ...
  await t.test("ONE mandate: ledger stores the exact object; verification RECOMPUTES the digest", async () => {
    ...
    const mandateDigest = sha2562(mandateCanonical(mandate));
    const ident = await authorityOwnerIdentity();
  ...
  await t.test("durable identity: a persisted keypair reloads \u2014 no fresh anonymous owner per runtime", async () => {
    ...
    const readOnly = { get: store.get, set: () => {
  ...
  await t.test("authority.issue enforces MISSION-CONTEXT equality", async () => {
    const r2 = await reachMcpCall("authority.issue", { missionId: "some-other-mission", scope: ["fs.read"] }, { missionId: "m-ctx", gate: GATE_OK });
    assert.equal(r2.decision, "refused");
    assert.match(r2.output, /does not match/);
  });
  ...
  await t.test("encrypted keystore: private keys persist ONLY sealed by the passphrase", async () => {
    ...
    assert.ok(!base.get().includes("byOwner"), "the stored blob is NOT plaintext key material");
...
test("VH-19 pipeline \u2014 the plane is wired, not an island", async (t) => {
  ...
  await t.test("pc tools are never category-bound (mission ctx only)", () => {
    const cats = ["code", "research", "data", "design"];
    for (const c of cats) {
  ...
  await t.test("executeTool refuses pc tools without a mission pc context", async () => {
    const r2 = await executeTool("pc.exec", { binary: "ls" }, { gate: async () => ({ approved: true, reason: "probe approves" }) });
    assert.equal(r2.outcome, "refused");
    assert.match(r2.output, /not attached/);
  });
  ...
  await t.test("the mission browser session persists AND adopts upgraded bindings", () => {
    ...
    const b2 = missionBrowser("m-session");
    assert.equal(a, b2, "same mission \u21D2 same live browser instance");
    assert.notEqual(a, missionBrowser("m-other"), "different mission \u21D2 isolated session");
    const fake = { open: async (u) => ({ url: u, status: 200, title: "t", bodyText: "", links: [], images: [] }), act: async (s) => s };
