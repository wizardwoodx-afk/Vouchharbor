...
var CONTRACT = " Finance contract: every figure names its register; every filing names its citation, period and due date; differences are itemised, never netted away; submissions ride the human gate.";
...
function main() {
  ...
  ok(
    "every entry is a finished professional (2 capabilities, 5+ keywords, doctrine + finance contract)",
    FINANCE_SPECIALISTS.every((s) => s.category === "finance" && s.capabilities.length === 2 && s.capabilities.every((c) => c.length > 25) && s.keywords.length >= 5 && s.riskTier === "safe" || s.riskTier === "risky" && /Finance contract:/.test(s.systemPrompt))
  );
  ok(
    "risk tiers honest \u2014 portal filings/payment runs are risky",
    SPECIALISTS.filter((s) => s.id.startsWith("finance.") && s.riskTier === "risky").length >= 25 && SPECIALISTS.find((s) => s.id === "finance.in.gstr3b-filer").riskTier === "risky"
  );
  const has = (id) => ids.has(id);
  ok(
    "GST core present (GSTR-1/3B/9, 2B recon, e-invoicing, e-way bill, notices, refunds, RCM, ITC)",
    has("finance.in.gstr1-filer") && has("finance.in.gstr3b-filer") && has("finance.in.gstr9-annual") && has("finance.in.gstr2b-recon") && has("finance.in.einvoice-irn") && has("finance.in.ewaybill") && has("finance.in.gst-notice-response") && has("finance.in.gst-refund") && has("finance.in.gst-rcm") && has("finance.in.itc-adjudicator")
  );
  ok(
    "TDS/income-tax present (24Q/26Q/27Q, 26AS/AIS, ITRs, capital gains, 15CA/CB, reassessment)",
    has("finance.in.tds-24q") && has("finance.in.tds-26q") && has("finance.in.tds-27q") && has("finance.in.tds-26as-recon") && has("finance.in.itr2-preparer") && has("finance.in.capital-gains-compute") && has("finance.in.15ca-cb") && has("finance.in.it-148-reassessment")
  );
  ok(
    "banking/recon present (UPI, NEFT/RTGS, gateway settlement, NACH, nostro, escrow, FX/FEMA, RBI reporting)",
    has("finance.in.upi-recon") && has("finance.in.neft-rtgs-recon") && has("finance.in.pg-settlement") && has("finance.in.nach-mandate") && has("finance.in.nostro-recon") && has("finance.in.escrow-recon") && has("finance.in.fx-fema") && has("finance.in.rbi-reporting")
  );
  ok(
    "books/close + audit + payroll + EXIM + fintech present",
    has("finance.in.tally-prime") && has("finance.in.monthly-close") && has("finance.in.bs-finalisation") && has("finance.in.tax-audit-44ab") && has("finance.in.indas115-reviewer") && has("finance.in.pf-epfo") && has("finance.in.payroll-processor") && has("finance.in.duty-drawback") && has("finance.in.sez-compliance") && has("finance.in.nbfc-iracp") && has("finance.in.co-lending") && has("finance.in.ppi-wallet")
  );
  ok(
    "MSME-focused present (43B(h), Udyam, CGTMSE pack, TReDS)",
    has("finance.in.msme-43bh") && has("finance.in.udyam-specialist") && has("finance.in.cgtmse-pack") && has("finance.in.treds-coordinator")
  );
  ok(
    "US GAAP/SEC + IFRS present (606/842/CECL/740, XBRL; IFRS 9/15/16, IAS 12)",
    has("finance.intl.asc606-revenue") && has("finance.intl.asc842-leases") && has("finance.intl.cecl-modeler") && has("finance.intl.asc740-tax") && has("finance.intl.xbrl-edgar") && has("finance.intl.ifrs9-financial") && has("finance.intl.ifrs15-revenue") && has("finance.intl.ias12-tax")
  );
  ok(
    "multi-jurisdiction tax present (US 1120/1040/SALT, UK VAT/CT, EU OSS/Pillar Two, APAC/GCC)",
    has("finance.intl.us-1120-preparer") && has("finance.intl.us-salt") && has("finance.intl.uk-vat-mtd") && has("finance.intl.eu-vat-oss") && has("finance.intl.eu-pillar2-globe") && has("finance.intl.sa-zatca") && has("finance.intl.jp-consumption")
  );
  ok(
    "AML/financial-crime present (KYC, monitoring, sanctions, SAR, TBML, travel rule)",
    has("finance.intl.kyc-cdd-analyst") && has("finance.intl.aml-monitoring") && has("finance.intl.sanctions-screening") && has("finance.intl.sar-drafting") && has("finance.intl.tbml-redflags") && has("finance.intl.crypto-travel-rule")
  );
  ok(
    "FP&A + treasury + audit intl + M&A present",
    has("finance.intl.rolling-forecast") && has("finance.intl.saas-metrics") && has("finance.intl.fx-hedge-accountant") && has("finance.intl.sox-404-tester") && has("finance.intl.pcaob-support") && has("finance.intl.qoe-analyst") && has("finance.intl.ppa-specialist")
  );
  ...
  ok(
    "finance prompt binds Reconciliation Proof + Statute-Dated Compliance playbooks",
