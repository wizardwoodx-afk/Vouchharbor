...
var BrainHostError = class extends Error {
};
function resolveBrainHarness(harnessId) {
  const spec = HARNESS_BY_ID.get(harnessId);
  if (!spec) return null;
  const path4 = typeof process !== "undefined" && process.env?.PATH ? process.env.PATH : "";
  const dirs = path4 ? path4.split(process.platform === "win32" ? ";" : ":") : [];
...
function bareProviderId(bin) {
  const parts = bin.split(/[\\/]/);
  return (parts[parts.length - 1] ?? bin).replace(/\.(exe|cmd|bat)$/i, "");
}
var realBrainInvoke = async (bin, argv, timeoutSecs) => {
  if (useTauri()) {
    const r = await ipc2.cliInvoke(bareProviderId(bin), argv.join(" "), void 0, timeoutSecs, argv);
...
async function runModelPrompt(prompt, harnessId, deps = realBrainDeps, timeoutSecs = 90) {
  ...
  try {
    ...
    const msg = e instanceof BrainHostError ? e.message : String(e?.message ?? e);
...
async function brainModelRun(objective, deps = realBrainDeps) {
  ...
  for (const id of prefOrder) {
    ...
    if (!r.refused.includes("not installed")) return r;
  }
  ...
  return {
    ...
    async decide(input, ctx) {
      ...
      return {
        ...plan,
        thoughts: [
          `brain: REAL model \u2014 ${r.harness.name} (${r.harness.id}) answered in ${r.durationMs}ms \xB7 labeled, this run \xB7 still gated by the same human gate`,
          ...plan.thoughts
        ],
...
var keyRef = (providerId) => `vh.providerkey.${providerId}`;
...
var PLANNER_SYSTEM = "You are the planner inside Vouch Harbor's governed brain. Answer with 3 to 6 concrete steps, one per line, no preamble or markdown. The runtime simulates risky steps and pauses them at a human gate \u2014 propose honestly.";
...
var USAGE_KEY = "vh.provider.usage";
var USAGE_CAP = 400;
...
function planFromModelText2(text, max = 6) {
  return text.split(/\r?\n/).map((l) => l.replace(/^\s*(?:[-*•]|\d+[.)])\s*/, "").trim()).filter((l) => l.length > 3).slice(0, max);
}
function wrapModelBrain(base, opts) {
  ...
  return {
    ...
    async decide(input, ctx) {
      ...
      const steps = planFromModelText2(r.text);
      if (steps.length === 0) {
...
function propose(input) {
  ...
  return { ...DEFAULT_ALLOW, audit: { kind: "policy", decision: "allow", rule: DEFAULT_ALLOW.rule, reason: DEFAULT_ALLOW.reason, tool: input.tool, ts: (/* @__PURE__ */ new Date()).toISOString() } };
}

// src/vouch/engine/toolSchema.ts
import { z } from "zod";
var WorkspaceWriteSchema = z.object({
  name: z.string().min(1).max(255).regex(/^[^/\\<>:"|?*\x00-\x1F]+$/, "filename must be a plain local name, no path separators or control chars"),
  content: z.string().max(1e6, "content exceeds 1 MB workspace-file limit")
});
var ShellExecSchema = z.object({
  command: z.string().min(1).max(4096),
  cwd: z.string().max(4096).optional()
});
var DispatchMissionSchema = z.object({
  objective: z.string().min(1).max(8e3)
});
var SystemInfoSchema = z.object({});
...
function formatParseErrors(result2) {
  ...
  return result2.error.issues.map((i) => `${i.path.filter((p) => typeof p === "string" || typeof p === "number").join(".") || "<args>"}: ${i.message}`).join("; ");
}
...
function emitToolResult(runId, threadId, toolCallId, result2, ok) {
  agui.emit({ type: "TOOL_CALL_RESULT", runId, threadId, ts: (/* @__PURE__ */ new Date()).toISOString(), toolCallId, result: result2, ok });
}
...
version: ${VH_VERSION} \xB7 brain: ${vouchBrain().id}`;
}
...
var DRILL_SCENARIOS = [
  {
    ...
    review: (repo) => {
      const src = fs2.existsSync(path3.join(repo, "guard.js")) ? fs2.readFileSync(path3.join(repo, "guard.js"), "utf8") : "";
  ...
  },
  {
    ...
    review: (repo) => {
      const src = fs2.existsSync(path3.join(repo, "maths.js")) ? fs2.readFileSync(path3.join(repo, "maths.js"), "utf8") : "";
  ...
  },
  {
    ...
    review: (repo) => {
      const src = fs2.existsSync(path3.join(repo, "vault.js")) ? fs2.readFileSync(path3.join(repo, "vault.js"), "utf8") : "";
  ...
  return {
    ...
    verify: async (cwd) => {
      if (canonicalTest && testFile && canary) {
        ...
        const current = fs2.existsSync(p) ? fs2.readFileSync(p, "utf8") : null;
        ...
        fs2.writeFileSync(p, canonicalTest);
      }
      ...
      if (isWriter) {
        if (sc.fix) {
          for (const [rel, content] of Object.entries(sc.fix)) {
            fs2.writeFileSync(path3.join(req.cwd, rel), content);
          }
        ...
        fs2.writeFileSync(vault, `module.exports = { OPEN: "open", CLOSE: "closed" };
`);
...
function makeRepo(sc) {
  const repo = fs2.mkdtempSync(path3.join(os2.tmpdir(), `vh-drill-${sc.id}-`));
  const testFile = scenarioTestFile(sc);
  ...
  if (canonicalTest) fs2.writeFileSync(path3.join(repo, "canonical-test.js"), canonicalTest);
...
function whichBin(name) {
  ...
  for (const d of dirs) {
    for (const ext of exts) {
      ...
      try {
        if (fs2.statSync(p).isFile()) return p;
      } catch {
      }
    }
  }
...
async function runDrill(scenarioId, opts) {
  ...
  try {
    ...
    if (!keep) fs2.rmSync(repo, { recursive: true, force: true });
